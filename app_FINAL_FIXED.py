...
def _reply_keyboard(chat_id, rows, actions=None):
    # Never use ForceReply or request the system typing keyboard. Telegram opens the
    # typing keyboard only when the user taps the text-entry field; menu screens use ReplyKeyboard.
...
def show_admin_home(chat_id, mid=None):
    ...
    style=_SETTINGS_CACHE.get("ui_style","crystal") if _SETTINGS_CACHE_READY else "crystal"
...
def _payment_message(chat_id, order):
    ...
    # A QR image from ZapUPI is the preferred payment screen. The ReplyKeyboard remains
    # visible; the system typing keyboard is not requested by the bot. It appears only
    # when the user explicitly taps the Telegram text-entry field.
    _clear_key_flow(chat_id)
    kb=_reply_keyboard(chat_id,rows,actions)
    if image_url:
...
def _send_user_key_detail_menu(chat_id,user_id,p,d,k):
    ...
    return _key_reply_send(chat_id,_key_detail_text(p,d,k),rows,actions,track=True)
...
def _start_main_menu(chat_id):
    ...
    welcome = _SETTINGS_CACHE.get("welcome_text", DEFAULT_WELCOME) if _SETTINGS_CACHE_READY else DEFAULT_WELCOME
    return _key_reply_send(chat_id, welcome or DEFAULT_WELCOME, rows, actions)
...
def _handle_key_reply_text(uid,cid,txt):
    ...
    if action.startswith("PAYURL:"):
        ...
        if not url:
            return True
        # Telegram ReplyKeyboard cannot directly carry a URL, so tapping this button
        # makes the bot send a normal clickable payment/deep-link message.
        safe_url=url.replace("&","&amp;").replace("\"","&quot;").replace("<","&lt;").replace(">","&gt;")
        tg_call("sendMessage",{"chat_id":cid,"text":f"💳 <a href=\"{safe_url}\">Open Payment</a>","parse_mode":"HTML","disable_web_page_preview":False})
        return True
...
def process_message_update(data):
    ...
    if uid in ADMIN_IDS and nav_txt in {"📦 Files","📢 Force Join","📣 Broadcast","👥 Users","⚙️ Settings","🔑 Keys","⚙️ /setup Content","📊 Statistics","🎨 UI Studio"}:
        ...
        if nav_txt=="📦 Files": show_files_menu(cid)
            ...
            _admin_begin_input(cid,uid,{"step":"WAIT_BC_CONTENT"},"📣 Please forward or send the message to broadcast:")
