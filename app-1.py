...
def deliver_file(chat_id, code):
    ...
    if not app_data:
        _track_public_result(chat_id, tg_call("sendMessage", {"chat_id": chat_id, "text": get_setting("error_feedback_text", DEFAULT_ERROR), "parse_mode": "Markdown"}), "error")
        return
...
def run_verify_process(chat_id, user_id, code, fj_mid):
    ...
    try:
        ...
        if anim_mid:
            # The user-message timer owns animation expiry. If that category is
            # disabled, preserve the old immediate-cleanup behaviour.
            if get_setting("cleanup_animation", "true").lower() != "true" and get_setting("cleanup_anim", "true").lower() == "true":
