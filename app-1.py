...
ADMIN_SESSIONS = {}
ADMIN_MENU_MESSAGES = {}  # admin chat_id -> latest bot menu message_id
...
def _admin_clear_menu(chat_id):
    ...
    if old:
        tg_call("deleteMessage", {"chat_id": chat_id, "message_id": old})
...
def _payment_message(chat_id, order):
    ...
    return _key_send(chat_id, {"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":buttons}})


def _key_lang_menu(chat_id):
    return _key_send(chat_id, {"chat_id":chat_id,"text":_key_text("key_lang"),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🇬🇧 English","callback_data":"klang:en"},{"text":"🇮🇳 Hindi","callback_data":"klang:hi"}]]}})
...
def _product_menu(chat_id, lang="en"):
    ...
    return _key_send(chat_id, {"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":rows}})
...
def _duration_menu(chat_id, pid):
    ...
    _key_send(chat_id, {"chat_id":chat_id,"text":_key_text("product_text", product=p.get("name","")),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":rows}})
...
def _my_keys(chat_id,user_id):
    ...
    if not active:
        return _key_send(chat_id,{"chat_id":chat_id,"text":_key_text("no_keys"),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🛒 Buy Keys","callback_data":"k_buy"}],[{"text":"🏠 Main Menu","callback_data":"k_home:en"}]]}})
...
def _send_user_key_detail_menu(chat_id,user_id,p,d,k):
    ...
    return _key_send(chat_id,{"chat_id":chat_id,"text":_key_detail_text(p,d,k),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":buttons}}, track=False)
...
def process_message_update(data):
    ...
    cmd=txt.strip().split(maxsplit=1)[0].split("@",1)[0].lower() if txt.strip() else ""
    if uid in ADMIN_IDS and uid in ADMIN_SESSIONS and not cmd.startswith("/"):
