    ...
    try:
        ...
        if is_admin_chat and isinstance(payload.get("reply_markup"), dict) and payload["reply_markup"].get("inline_keyboard"):
            ...
            if method == "editMessageText" and payload.get("message_id"):
                # Telegram cannot attach a ReplyKeyboardMarkup while editing
                # an existing message. Remove the old detail message and post
                # the new detail text above the native keyboard instead.
                old_mid = payload.pop("message_id")
                try:
                    _TG_SESSION.post(
                        f"https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage",
                        json={"chat_id": chat_id, "message_id": old_mid}, timeout=(2.5, 8)
                    )
                except Exception:
                    pass
                method = "sendMessage"
...
def show_admin_home(chat_id, mid=None):
    ...
    if mid:
        try:
            ...
            pass
    data={"chat_id":chat_id,"text":txt,"reply_markup":_admin_reply_keyboard()}
    if entities:
