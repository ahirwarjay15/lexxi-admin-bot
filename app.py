...
def run_verify_process(chat_id, user_id, code, fj_mid):
    ...
    try:
        ...
        # Replace the previous UI with the current list of channels still
        # pending.  Only pending channels are shown on every retry.
        cleanup_verification_state(chat_id, user_id, code, include_base=True)
        failed_on = get_setting("verify_failed_enabled", "true").lower() == "true"
        retry_on = get_setting("verify_retry_enabled", "true").lower() == "true"
        mids = {}
        markup = {"reply_markup": {"inline_keyboard": _fj_keyboard(unjoined, code)}}
        if failed_on:
            r = tg_call("sendMessage", {
                "chat_id": chat_id,
                "text": get_setting("verify_failed_text", DEFAULT_VERIFY_FAILED),
                "parse_mode": "Markdown",
                **markup
            })
            if r.get("ok"):
                mids["failed"] = r["result"]["message_id"]
        if retry_on:
            r = tg_call("sendMessage", {
                "chat_id": chat_id,
            ...
            if r.get("ok"):
                mids["retry"] = r["result"]["message_id"]
...
def process_callback_query(cq):
    ...
    if uid in ADMIN_IDS:
        ...
        if cdata == "adm_home":
            ...
            ADMIN_SESSIONS[uid]={"step":"SET_VERIFY_FAILED"}; cur=get_setting("verify_failed_text", DEFAULT_VERIFY_FAILED); tg_call("sendMessage",{"chat_id":cid,"text":f"📝 CURRENT MESSAGE:\n\n{cur}\n\n✏️ Send the new Verification Failed message:"})
...
def webhook():
    ...
    # Message Router. Admin commands/settings are handled directly so /admin
    # can never be queued behind public delivery/verification work. Public
    # messages remain asynchronous to keep the webhook fast.
    if "message" in data:
...
def process_message_update(data):
    ...
    if txt.startswith("/start"):
        ...
        if len(parts) > 1:
            ...
            elif send_fj(cid, uid, code, uname):
                deliver_file(cid, code)
        else:
