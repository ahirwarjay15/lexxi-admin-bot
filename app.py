...
# Dedicated pools keep user commands/UI responsive even when profile/DB jobs are slow.
_COMMAND_EXECUTOR = ThreadPoolExecutor(max_workers=8, thread_name_prefix="command")
_PROFILE_EXECUTOR = ThreadPoolExecutor(max_workers=16, thread_name_prefix="profile")
_PAYMENT_EXECUTOR = ThreadPoolExecutor(max_workers=8, thread_name_prefix="payment")
_PAYMENT_INFLIGHT = set()
_PAYMENT_INFLIGHT_LOCK = threading.Lock()
...
USER_PROFILE_CACHE = {}  # user_id -> latest Telegram details known to the bot
...
def tg_call(method, data):
    try:
        # Normalize every admin menu centrally. This catches both newer and
        # legacy menu builders, guarantees a Back button, and applies the
        # per-button UI Studio settings consistently.
        ...
        if admin_chat:
            ...
            # Multi-step admin editors must always expose a safe way out, even
            # when an older step sends a plain text prompt instead of a menu.
...
def _load_settings_cache():
    """Start a single background settings load; NEVER block webhook workers on Supabase."""
...
def get_setting(key, default=""):
    # Never make a Telegram webhook worker wait for Supabase. If the cache is
    # still loading, return the caller's safe default and let the background
    # loader populate the real values for subsequent requests.
...
def get_unjoined(user_id):
    ...
    if supabase and method != "joined":
        try:
            # A join request itself counts as verified. Do not read an optional
            # `status` column because older schemas may not have it.
...
def _admin_menu_lock(chat_id):
    # Per-admin-chat lock prevents two fast webhook workers from leaving two
    # menu messages visible at the same time.
...
def _admin_clear_menu(chat_id):
    ...
    with lock:
        ...
        _delete_admin_menu_ids(chat_id, mids)

def _admin_session_back_callback(session):
    """Return the most useful parent menu for an active admin input session."""
    st=str((session or {}).get("step") or "")
    if st.startswith("UI_BUTTON"):
        return "ui_buttons"
    if st.startswith("K_"):
        pid=(session or {}).get("pid")
        if pid and st not in {"K_ADD_PRODUCT", "K_ADD_KEYS_GLOBAL", "K_PAYMENT_SETTINGS"}:
            return f"kap:{pid}"
        return "m_keys"
...
def _admin_add_back_button(data, user_id=None):
    """Guarantee a Back button and UI styling on every admin inline menu."""
    ...
    # Re-run the centralized per-button customization after adding Back.
    try:
        data=dict(data)
...
def _admin_begin_input(chat_id, user_id, session, prompt):
    # Input prompts also get an explicit Back/Cancel action.
...
def _admin_store_menu(chat_id, result):
    if isinstance(result, dict) and result.get("ok"):
        ...
        if mid:
            lock = _admin_menu_lock(chat_id)
            with lock:
                # Last completed menu wins. If another worker sent a menu
                # milliseconds earlier, delete that newer tracked menu before
                # registering this one. This closes the send/store race.
                ...
                _delete_admin_menu_ids(chat_id, old_mids)
...
def _remove_legacy_reply_keyboard(chat_id):
    """Remove InlineKeyboard left by older bot versions."""
    try:
        r=tg_call("sendMessage",{"chat_id":chat_id,"text":"\u2063","reply_markup":{"remove_keyboard":True}})
        mid=(r.get("result") or {}).get("message_id") if r.get("ok") else None
        if mid: tg_call("deleteMessage",{"chat_id":chat_id,"message_id":mid})
    except Exception:
        pass

# Per-button UI registry. Telegram supports native Primary/Success/Danger styles
# for inline buttons, but not arbitrary HEX/gradient colors. Individual button
# settings are persisted through the existing settings table.
_UI_BUTTON_REGISTRY = {}
_UI_BUTTON_REGISTRY_LOCK = threading.Lock()

def _button_token(btn):
    # Include label + action so two different buttons pointing to the same URL
    # (or using related callbacks) still receive independent settings.
    raw = "|".join([
        str(btn.get("text") or ""),
...
def _ui_button_style(role="default", fallback="primary", token=None):
    ...
    # A per-button override wins; an empty/missing override falls back to the
    # role-wide color selected in UI Studio. This keeps both global and
    # individual customization working together.
    if token:
...
def _style_inline_keyboard(kb, role_default="default"):
    ...
    for row in kb or []:
        ...
        for btn in row or []:
            ...
            elif any(x in cb for x in ("k_order:","kp_verify:","k_buy","k_my","k_prod:","k_dur:","klang:","k_home:")): role="buy"
...
def _key_inline_send(chat_id,text,rows,actions,track=True):
    _clear_key_flow(chat_id); _remove_legacy_reply_keyboard(chat_id)
    kb=[]
    for row in rows or []:
        nr=[]
        for label in row or []:
...
def show_admin_home(chat_id, mid=None):
    ...
    _remove_legacy_reply_keyboard(chat_id)
    if mid: tg_call("deleteMessage",{"chat_id":chat_id,"message_id":mid})
    else: _admin_clear_menu(chat_id)
...
def healthz():
    return {"status": "ok", "telegram_configured": bool(BOT_TOKEN), "supabase_configured": bool(supabase), "zapupi_configured": bool(os.environ.get("ZAPUPI_API_KEY", "").strip())}, 200
...
def _new_id(prefix):
    # Cryptographically random suffix prevents collisions even when many
    # workers generate IDs within the same millisecond.
...
def _ensure_key_defaults():
    ...
    # Default manual/fallback UPI configuration. ZapUPI remains the primary
    # payment provider; this VPA is used only when a direct UPI fallback is needed.
    if not get_setting("payment_upi_id", "").strip():
        set_setting("payment_upi_id", "lexxiowner@upi")
    if not get_setting("payment_payee_name", "").strip():
        set_setting("payment_payee_name", "Lexxi")
...
def _base_public_url():
    raw = (os.environ.get("RENDER_EXTERNAL_URL") or os.environ.get("WEBHOOK_URL") or os.environ.get("TELEGRAM_WEBHOOK_URL") or "").strip().rstrip("/")
    for suffix in ("/payment/webhook", "/webhook"):
        if raw.endswith(suffix):
            raw = raw[:-len(suffix)].rstrip("/")
            break
    return raw

def _zapupi_webhook_url():
    base = _base_public_url()
    return base + "/payment/webhook" if base else ""

def _zapupi_create_order(order):
    zap_key = os.environ.get("ZAPUPI_API_KEY", "").strip()
    if not zap_key:
        return None, "zapupi_not_configured"
    webhook_url = _zapupi_webhook_url()
    payload = {
        "zap_key": zap_key,
        "order_id": str(order["id"]),
        "amount": f"{float(order['amount']):.2f}",
        "remark": f"LEXI Key | {order['user_id']} | {order['product_id']} | {order['duration_id']}",
    }
    if webhook_url:
        payload["webhook_url"] = webhook_url
    try:
        rr = requests.post("https://pay.zapupi.com/api/create-order", json=payload, timeout=10)
        body = rr.json() if rr.ok else {}
        if not isinstance(body, dict) or str(body.get("status", "")).lower() != "success":
            return None, str(body.get("message") or "ZapUPI order creation failed")
        data = body.get("data") if isinstance(body.get("data"), dict) else {}
        payment_url = data.get("payment_url") or body.get("payment_url")
        payment_id = data.get("pay_id") or data.get("payment_id") or body.get("pay_id")
        # Custom API mode may return a ready-made QR image + direct UPI/Paytm intents.
        # Keep these optional so the normal ZapUPI hosted payment page remains a safe fallback.
        payment_image_url = data.get("payment_image_url") or body.get("payment_image_url")
        upi_button = data.get("upi_button") or body.get("upi_button") or data.get("payment_data") or body.get("payment_data")
        paytm_button = data.get("paytm_button") or body.get("paytm_button")
        if not payment_url and not payment_image_url:
            return None, "ZapUPI did not return a payment page or QR image"
        return {
            "payment_url": str(payment_url) if payment_url else None,
            "payment_id": str(payment_id) if payment_id else None,
            "payment_image_url": str(payment_image_url) if payment_image_url else None,
            "upi_button": str(upi_button) if upi_button else None,
            "paytm_button": str(paytm_button) if paytm_button else None,
        }, None
    except Exception as exc:
        print(f"ZAPUPI CREATE ORDER ERROR: {exc}", flush=True)
        return None, "ZapUPI connection error"

def _set_order_cancelled(order_id, reason="payment_create_failed"):
    with KEY_DATA_LOCK:
        orders = key_orders()
        for o in orders:
            if o.get("id") == order_id and o.get("status") == "PENDING":
                o["status"] = "CANCELLED"; o["cancel_reason"] = reason; o["cancelled_at"] = time.time()
        _save_orders(orders)
...
def _new_order(user_id, pid, did):
    with KEY_DATA_LOCK:
        ...
        if not _available_keys(d):
            return None, "empty"
        try:
            amount = round(float(d.get("price", 0)), 2)
        except Exception:
            return None, "invalid_price"
        if not math.isfinite(amount) or amount <= 0:
            return None, "invalid_price"
        # secrets makes the ZapUPI order id practically collision-free across workers.
        order = {
            "id": "LEXI" + str(int(time.time() * 1000)) + secrets.token_hex(4),
            ...
            "payment_ref": _new_id("PAY"), "txn_ref": None, "payment_id": None, "payment_url": None,
        }
        orders = key_orders(); orders.append(order); _save_orders(orders)
    # Never hold the inventory lock while waiting on an external payment API.
    zap_key = os.environ.get("ZAPUPI_API_KEY", "").strip()
    if zap_key:
        payment, err = _zapupi_create_order(order)
        if not payment:
            _set_order_cancelled(order["id"], err)
            return None, err
        with KEY_DATA_LOCK:
            orders = key_orders()
            saved = next((o for o in orders if o.get("id") == order["id"]), None)
            if not saved or saved.get("status") != "PENDING":
                return None, "order_not_available"
            saved["payment_url"] = payment.get("payment_url")
            if payment.get("payment_id"):
                saved["payment_id"] = payment["payment_id"]
            if payment.get("payment_image_url"):
                saved["payment_image_url"] = payment["payment_image_url"]
            if payment.get("upi_button"):
                saved["upi_button"] = payment["upi_button"]
            if payment.get("paytm_button"):
                saved["paytm_button"] = payment["paytm_button"]
            _save_orders(orders)
            order = saved
    return order, None
...
def _mark_order_paid(order_id, amount, txn_ref, payment_id=None):
    with KEY_DATA_LOCK:
        ...
        if round(float(order.get("amount", 0)), 2) != round(float(amount), 2):
            return None, "amount_mismatch"
        if not str(txn_ref or "").strip():
            return None, "missing_transaction_reference"
...
def _qr_link(amount, order_id, payment_ref):
    ...
    upi_id = get_setting("payment_upi_id", "lexxiowner@upi").strip() or "lexxiowner@upi"
    payee = get_setting("payment_payee_name", "Lexxi").strip() or "Lexxi"
    if not upi_id:
        return ""
...
def _payment_message(chat_id, order):
    # Prefer ZapUPI custom QR when available; otherwise keep the hosted payment URL.
    image_url = str(order.get("payment_image_url") or "").strip()
    payment_url = str(order.get("payment_url") or "").strip()
    upi_url = unquote(str(order.get("upi_button") or "").strip())
    paytm_url = unquote(str(order.get("paytm_button") or "").strip())
    if not upi_url and not paytm_url and not image_url:
        upi_url = _qr_link(order["amount"], order["id"], order["payment_ref"])

    rows=[]; actions={}
    if upi_url:
        label="📲 𝗣𝗔𝗬 𝗩𝗜𝗔 𝗨𝗣𝗜"; rows.append([label]); actions[label]="PAYURL:"+upi_url
    elif payment_url:
        label="💳 𝗢𝗣𝗘𝗡 𝗣𝗔𝗬𝗠𝗘𝗡𝗧"; rows.append([label]); actions[label]="PAYURL:"+payment_url
    if paytm_url:
        label="💠 𝗣𝗔𝗬 𝗩𝗜𝗔 𝗣𝗔𝗬𝗧𝗠"; rows.append([label]); actions[label]="PAYURL:"+paytm_url
    ...
    # A QR image from ZapUPI is the preferred payment screen. Inline buttons only.
    ...
    for row in rows:
        nr=[]
        for label in row:
            action=str(actions.get(label,"")); b={"text":str(label)}
            if action.startswith("PAYURL:"):
                b["url"]=action[7:].strip(); b["style"]=_ui_button_style("buy","success")
            else:
                b["callback_data"]=action[:64]; back="BACK" in str(label).upper()
                b["style"]=_ui_button_style("back" if back else "buy","danger" if back else "success")
            nr.append(b)
        kb.append(nr)
    kb={"inline_keyboard":kb}
    if image_url:
        r=tg_call("sendPhoto",{"chat_id":chat_id,"photo":image_url,"caption":text,"parse_mode":"Markdown","reply_markup":kb})
        if r.get("ok"):
            KEY_FLOW_MESSAGES.setdefault(chat_id,set()).add(r["result"]["message_id"])
            return r
    # Fallback: normal text payment page/deep-link.
    if not image_url and not (payment_url or upi_url or paytm_url):
        text += "\n\n⚠️ Payment method is not configured yet."
    return _key_send(chat_id,{"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":kb})

def _key_lang_menu(chat_id):
    rows=[["🇬🇧 𝗘𝗡𝗚𝗟𝗜𝗦𝗛","🇮🇳 𝗛𝗜𝗡𝗗𝗜"]]
    return _key_inline_send(chat_id,_key_text("key_lang"),rows,{rows[0][0]:"klang:en",rows[0][1]:"klang:hi"})

def _key_home(chat_id, lang="en"):
    rows=[["🛒 𝗕𝗨𝗬 𝗞𝗘𝗬𝗦"],["🔑 𝗠𝗬 𝗞𝗘𝗬𝗦"],["🏠 𝗠𝗔𝗜𝗡 𝗠𝗘𝗡𝗨"]]
    return _key_inline_send(chat_id,_key_text("key_home_en" if lang=="en" else "key_home_hi"),rows,
...
def _duration_menu(chat_id,pid):
    ...
    return _key_inline_send(chat_id,_key_text("product_text",product=p.get("name","")),rows,actions)
...
def _duration_details(chat_id,pid,did):
    ...
    rows=[["💳 𝗕𝗨𝗬 𝗡𝗢𝗪"],["🔙 𝗕𝗔𝗖𝗞"]]
    return _key_inline_send(chat_id,_key_text("duration_text",duration=d.get("name"),price=d.get("price")),rows,
...
def _my_keys(chat_id,user_id):
    ...
    if not active:
        ...
        return _key_inline_send(chat_id,_key_text("no_keys"),rows,{rows[0][0]:"k_buy",rows[1][0]:"k_home:en"})
...
def _send_user_key_detail_menu(chat_id,user_id,p,d,k):
    ...
    return _key_inline_send(chat_id,_key_detail_text(p,d,k),rows,actions,track=True)
...
def _handle_key_callback(cq):
        ...
        if zap_key:
            try:
                ...
                if str(vd.get("status","")).lower()=="success":
                    ...
                    if result:
                        _notify_paid_and_deliver(result); return True
    ...
    if cdata.startswith("k_reset:") or cdata.startswith("k_delete:"):
        ...
        if action == "reset":
            max_resets = int(get_setting("key_max_resets", "3"))
            current_resets = int(k.get("resets", 0))
            if days is None or days < 7 or current_resets >= max_resets:
                tg_call("sendMessage", {"chat_id": cid, "text": "⚠️ Reset is unavailable for this key."}); return True
        reqs=_json_setting("key_requests_v1",[])
        if any(r.get("status") == "PENDING" and r.get("action") == action and str(r.get("key_id")) == str(kid) for r in reqs):
            tg_call("sendMessage", {"chat_id": cid, "text": "⏳ A request for this key is already pending."}); return True
        reqs.append({"id":_new_id("REQ"),"action":action,"user_id":uid,"key_id":kid,"product":p.get("name"),"duration":d.get("name"),"created_at":time.time(),"status":"PENDING"}); _save_json_setting("key_requests_v1",reqs)
...
def _keys_admin_callback(cq):
    ...
    if c.startswith("kar_"):
        ...
        _key_admin_request_menu(cid,mid); return True
    if c=="ka_payment":
        _admin_begin_input(cid,uid,{"step":"K_PAYMENT_SETTINGS"},"Send: `UPI ID | Payee Name`\nExample: `lexxiowner@upi | Lexxi`"); return True
    if c=="ka_messages":
...
def _notify_paid_and_deliver(result):
    order, p, d, k = result
    ...
    if not _available_keys(d):
        _admin_notice(_key_text("stock_zero_admin", product=p.get("name"), duration=d.get("name")))
    user_id = int(order["user_id"])
    _clear_key_flow(user_id)
    _send_user_key_message(user_id, p, d, k, pin=True)

def _process_zapupi_webhook(data):
    order_id = str(data.get("order_id", "")).strip()
    try:
        status = str(data.get("status", "")).strip().lower()
        env = str(data.get("environment", "")).strip().lower()
        if not order_id or env == "test" or status != "success":
            return
        zap_key = os.environ.get("ZAPUPI_API_KEY", "").strip()
        if not zap_key:
            print(f"ZAPUPI CONFIG MISSING order={order_id}", flush=True)
            return
        rr = requests.post(
            "https://pay.zapupi.com/api/order-status",
            json={"zap_key": zap_key, "order_id": order_id}, timeout=(3, 8)
        )
        body = rr.json() if rr.ok else {}
        vd = body.get("data", {}) if isinstance(body, dict) else {}
        if str(vd.get("status", "")).lower() != "success":
            print(f"ZAPUPI STATUS NOT SUCCESS order={order_id} status={vd.get('status')}", flush=True)
            return
        amount = float(vd.get("pay_amount", vd.get("amount", 0)) or 0)
        txn = str(vd.get("txn_id") or vd.get("utr") or data.get("txn_id") or data.get("utr") or "").strip()
        result, err = _mark_order_paid(order_id, amount, txn, vd.get("txn_id") or data.get("txn_id"))
        if not result:
            print(f"ZAPUPI FULFILLMENT SKIPPED order={order_id}: {err}", flush=True)
            return
        _notify_paid_and_deliver(result)
    except Exception as exc:
        print(f"ZAPUPI WEBHOOK ERROR order={order_id}: {exc}", flush=True)
    finally:
        if order_id:
            with _PAYMENT_INFLIGHT_LOCK:
                _PAYMENT_INFLIGHT.discard(order_id)

@app.route("/payment/webhook", methods=["POST", "GET"])
def zapupi_payment_webhook():
    # ZapUPI requires HTTP 200 + JSON acknowledgement.  ACK immediately so a
    # slow order-status/DB/Telegram operation can never make ZapUPI retry.
    if request.method == "GET":
        return {"status": "ok", "service": "zapupi-webhook"}, 200
    data = request.get_json(silent=True) or {}
    order_id = str(data.get("order_id", "")).strip()
    if order_id:
        with _PAYMENT_INFLIGHT_LOCK:
            if order_id not in _PAYMENT_INFLIGHT:
                _PAYMENT_INFLIGHT.add(order_id)
                try:
                    _PAYMENT_EXECUTOR.submit(_safe_run, _process_zapupi_webhook, data)
                except Exception as exc:
                    _PAYMENT_INFLIGHT.discard(order_id)
                    print(f"ZAPUPI QUEUE ERROR order={order_id}: {exc}", flush=True)
...
def _keys_admin_message_step(uid,cid,txt,msg):
    ...
    if st=="K_PAYMENT_SETTINGS":
        ...
        if len(parts)>=1 and parts[0]: set_setting("payment_upi_id",parts[0])
        elif len(parts)==0 or not parts[0]: set_setting("payment_upi_id","lexxiowner@upi")
        if len(parts)>=2 and parts[1]: set_setting("payment_payee_name",parts[1])
        elif len(parts)<2 or not parts[1]: set_setting("payment_payee_name","Lexxi")
        ADMIN_SESSIONS.pop(uid,None); tg_call("sendMessage",{"chat_id":cid,"text":"✅ Payment UPI settings saved. ZapUPI authentication comes from Render `ZAPUPI_API_KEY`."}); _keys_admin_menu(cid); return True
    return False
...
def _key_command_wrapper(data):
    ...
    if cmd=="/buykeys":
        print(f"KEY COMMAND START user={uid} chat={cid} action=buykeys", flush=True)
        try:
            _start_buykeys(cid)
            print(f"KEY COMMAND DONE user={uid} chat={cid} action=buykeys", flush=True)
        except Exception as exc:
...
def process_callback_query(cq):
    ...
    if uid in ADMIN_IDS and c in {"adm_home", "m_files", "m_fj", "m_bc", "m_users", "m_sett", "m_keys", "m_setup", "ui_buttons", "m_ui"}:
        # Any explicit navigation away from an editor cancels its pending text
        # input so the next admin message cannot be consumed by the old step.
        ADMIN_SESSIONS.pop(uid, None)
    ...
    if uid in ADMIN_IDS and c=="hide_admin":
        ADMIN_SESSIONS.pop(uid,None); _admin_clear_menu(cid); _remove_legacy_reply_keyboard(cid); return "OK",200
...
def process_message_update(data):
    ...
    # Cache the sender immediately for username -> ID lookups; persistence is async.
    if uid:
    ...
    cmd=txt.strip().split(maxsplit=1)[0].split("@",1)[0].lower() if txt.strip() else ""
    # Admin-only user lookup tools. These run before any multi-step admin session.
    ...
    # Voluntary phone sharing: Telegram only sends a phone number when the user
    # explicitly shares a contact. It is never inferred from a Telegram ID.
    ...
    if contact and contact.get("user_id") and int(contact.get("user_id")) == int(uid):
        ...
        return "OK", 200
    # Explicit per-button label editor. Telegram does not expose a long-press event to bots.
    ...
    # Menu navigation is inline-only now. Legacy inline-keyboard text is ignored.
    if _key_command_wrapper(data):
        return "OK",200
    result = _original_process_message_update(data)
    # Anything that reaches here was not consumed by a known public/admin route.
    # Do not forward admin workflow input; those are handled above.
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        uid, cid, mid, cdata = cq["from"]["id"], cq["message"]["chat"]["id"], cq["message"]["message_id"], cq.get("data", "")
        _COMMAND_EXECUTOR.submit(_safe_run, tg_call, "answerCallbackQuery", {"callback_query_id": cq["id"]})
    ...
    # Message Router. Never run message handling in the DB/update executor:
    # that pool also handles background bookkeeping and can become saturated.
    # Critical commands/UI go to a dedicated command pool; ordinary public
    # messages go to the public pool. The webhook itself returns immediately.
    if "message" in data:
        ...
        try:
            ...
            print(f"MESSAGE QUEUED user={uid} command={cmd or '-'} critical={critical}", flush=True)
        except Exception as exc:
            print(f"MESSAGE QUEUE ERROR user={uid} text={txt[:80]!r}: {exc}", flush=True)
            # Last-resort synchronous fallback only when queue submission fails.
...
def _remember_user_profile(msg):
    """Remember only Telegram data actually exposed by the Bot API/update."""
    ...
    if supabase:
        try:
            # Keep compatibility with the existing users schema.
...
def _track_user(uid, username, first_name, last_name="", language_code="", is_premium=False, chat_id=None):
    ...
    try:
        ...
        try:
            ...
            # Older schema compatibility: only rely on known columns.
            supabase.table("users").upsert(base).execute()
    except Exception as exc:
        print(f"USER TRACK ERROR user={uid}: {exc}", flush=True)
...
def _get_stored_phone(uid):
    # Phone is only available if the user voluntarily shared it with the bot.
    try:
...
def _lookup_user_by_username(username):
    ...
    try:
        ...
        print(f"USERNAME LOOKUP ERROR @{username}: {exc}", flush=True)
    return None


def _user_details_text(uid, seed=None, source="lookup"):
    """Build maximum details available to a Telegram Bot, without inventing private data."""
    uid = int(uid)
    prof = dict(USER_PROFILE_CACHE.get(uid, {}))
    if seed:
        prof.update({k:v for k,v in seed.items() if v not in (None, "")})

    # Enrich from Telegram while the bot can access this private chat.
    chat = {}
    try:
        ...
        print(f"USER DETAIL getChat ERROR user={uid}: {exc}", flush=True)
...
def _forward_invalid_to_admins(msg, reason="Invalid / unrecognized message"):
    """Forward the original update plus maximum available sender metadata to admins."""
    if not ADMIN_IDS:
        return
    ...
    for aid in ADMIN_IDS:
        # Original message first, so media/caption/sticker/etc. stays intact when forwarding is allowed.
        try:
            ...
            print(f"INVALID FORWARD ERROR admin={aid} user={uid}: {exc}", flush=True)
    ...
    if txt.startswith("/start"):
        ...
        try:
            ...
            if code:
                ...
                # My Keys as InlineKeyboard buttons.
