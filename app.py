...
def show_admin_home(chat_id, mid=None):
    ...
    reply_kb={"keyboard":[[{"text":"📦 Files"},{"text":"📢 Force Join"}],[{"text":"📣 Broadcast"},{"text":"👥 Users"}],[{"text":"⚙️ Settings"},{"text":"🔑 Keys"}],[{"text":"⚙️ /setup Content"},{"text":"📊 Statistics"}],[{"text":"🚫 Hide Admin Panel"}]],"resize_keyboard":True,"is_persistent":True}
    if mid:
        tg_call("deleteMessage", {"chat_id": chat_id, "message_id": mid})
...
def _payment_message(chat_id, order):
    ...
    buttons = []
    if link:
        buttons.append([{"text":"💳 Pay via UPI", "url":link}])
    buttons.append([{"text":"🔎 Verify Payment", "callback_data":"kp_verify:"+order["id"]}])
    buttons.append([{"text":"🔄 New Payment Order", "callback_data":"kp_retry:"+order["id"]}])
    text = _key_text("payment_pending", order_id=order["id"], amount=order["amount"])
    if link:
        text += "\n\n⚠️ Pay the exact amount for this order. The QR/link alone does not confirm payment."
    else:
        text += "\n\n⚠️ Admin payment UPI is not configured yet."
    return tg_call("sendMessage", {"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":buttons}})


def _key_lang_menu(chat_id):
    return tg_call("sendMessage", {"chat_id":chat_id,"text":_key_text("key_lang"),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🇬🇧 English","callback_data":"klang:en"},{"text":"🇮🇳 Hindi","callback_data":"klang:hi"}]]}})


def _key_home(chat_id, lang="en"):
    text = _key_text("key_home_en" if lang == "en" else "key_home_hi")
    return tg_call("sendMessage", {"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🛒 Buy Keys","callback_data":"k_buy"}],[{"text":"🔑 My Keys","callback_data":"k_my"}]]}})
...
def _product_menu(chat_id, lang="en"):
    ...
    rows=[]
    for p in products:
        # Product button contains only product name + emoji; stock count is never exposed.
        rows.append([{"text":f"{p.get('emoji','📦')} {p.get('name','Product')}","callback_data":"k_prod:"+str(p.get("id"))}])
    rows.append([{"text":"⬅️ Back","callback_data":"k_home:"+lang}])
    text="🛒 **BUY KEYS**\n\nSelect a product:" if products else "🛒 **BUY KEYS**\n\nNo products are available yet."
    return tg_call("sendMessage", {"chat_id":chat_id,"text":text,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":rows}})
...
def _duration_menu(chat_id, pid):
    ...
    rows=[]
    for d in p.get("durations",[]):
        if not d.get("enabled",True) or not _available_keys(d):
            continue
        # Button contains ONLY emoji + duration name; price is in the text above.
        rows.append([{"text":f"{d.get('emoji','📅')} {d.get('name','1 Day')}","callback_data":"k_dur:"+str(pid)+":"+str(d.get("id"))}])
    rows.append([{"text":"⬅️ Back","callback_data":"k_buy"}])
    tg_call("sendMessage", {"chat_id":chat_id,"text":_key_text("product_text", product=p.get("name","")),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":rows}})
...
def _duration_details(chat_id,pid,did):
    ...
    tg_call("sendMessage", {"chat_id":chat_id,"text":_key_text("duration_text",duration=d.get("name"),price=d.get("price")),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"💳 Buy Now","callback_data":f"k_order:{pid}:{did}"}],[{"text":"⬅️ Back","callback_data":f"k_prod:{pid}"}]]}})


def _my_keys(chat_id,user_id):
    active=_active_user_key(user_id)
    if not active:
        return tg_call("sendMessage",{"chat_id":chat_id,"text":_key_text("no_keys"),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🛒 Buy Keys","callback_data":"k_buy"}],[{"text":"🏠 Main Menu","callback_data":"k_home:en"}]]}})
    # Newest purchase first.
    active.sort(key=lambda x: float(x[2].get("purchased_at",0)), reverse=True)
    if len(active)==1:
        p,d,k=active[0]
        return _send_user_key_detail_menu(chat_id,user_id,p,d,k)
    rows=[]
    for p,d,k in active:
        rows.append([{"text":f"{d.get('emoji','📅')} {d.get('name','Key')} • {_format_dt(k.get('purchased_at'))}","callback_data":"k_view:"+str(k.get("id"))}])
    rows.append([{"text":"🛒 Buy Keys","callback_data":"k_buy"}])
    return tg_call("sendMessage",{"chat_id":chat_id,"text":"🔑 **MY KEYS**\n\nLatest purchase is shown first. Select a key:","parse_mode":"Markdown","reply_markup":{"inline_keyboard":rows}})


def _send_user_key_detail_menu(chat_id,user_id,p,d,k):
    days=_duration_days(d.get("name",""))
    can_reset=(days is not None and days>=7) or float(d.get("days",0) or 0)>=7
    resets=int(k.get("resets",0)); max_resets=int(get_setting("key_max_resets","3"))
    buttons=[]
    if can_reset and resets < max_resets:
        buttons.append([{"text":"🔄 Reset Device","callback_data":"k_reset:"+str(k.get("id"))}])
    buttons.append([{"text":"🗑 Delete Key","callback_data":"k_delete:"+str(k.get("id"))}])
    buttons.append([{"text":"⬅️ My Keys","callback_data":"k_my"}])
    return tg_call("sendMessage",{"chat_id":chat_id,"text":_key_detail_text(p,d,k),"parse_mode":"Markdown","reply_markup":{"inline_keyboard":buttons}})
...
def _handle_key_callback(cq):
    ...
    if cdata.startswith("kp_verify:"):
        ...
        if order.get("status")=="PAID":
            ...
            if p and d and k: _send_user_key_message(cid,p,d,k); return True
        if float(order.get("expires_at",0))<time.time():
            tg_call("sendMessage",{"chat_id":cid,"text":_key_text("payment_failed")}); return True
        tg_call("sendMessage",{"chat_id":cid,"text":"⏳ Payment is not verified yet. Please wait for the payment notification, then tap Verify again."}); return True
...
def _keys_admin_menu(cid,mid=None):
    ...
    tg_call("editMessageText" if mid else "sendMessage",{"chat_id":cid,**({"message_id":mid} if mid else {}),"text":txt,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(kb)}})
...
def _keys_admin_products(cid,mid=None):
    ...
    tg_call("editMessageText" if mid else "sendMessage",{"chat_id":cid,**({"message_id":mid} if mid else {}),"text":"📦 **PRODUCTS**\n\nNo default products are created.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(rows)}})
...
def _keys_admin_product(cid,pid,mid=None):
    ...
    tg_call("editMessageText" if mid else "sendMessage",{"chat_id":cid,**({"message_id":mid} if mid else {}),"text":f"📦 **{p.get('name')}**\n\nConfigure durations below.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(rows)}})
...
def _key_admin_request_menu(cid,mid=None):
    ...
    tg_call("editMessageText" if mid else "sendMessage",{"chat_id":cid,**({"message_id":mid} if mid else {}),"text":f"📨 **KEY REQUESTS**\n\nPending: `{len(pending)}`","parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(rows)}})
...
def process_message_update(data):
    ...
    if uid in ADMIN_IDS and uid in ADMIN_SESSIONS:
        ...
        cmd=txt.strip().split(maxsplit=1)[0].lower() if txt.strip() else ""
        if cmd=="/keymsg":
    ...
    if uid in ADMIN_IDS and txt.strip() in {"📦 Files","📢 Force Join","📣 Broadcast","👥 Users","⚙️ Settings","🔑 Keys","⚙️ /setup Content","📊 Statistics"}:
        ADMIN_SESSIONS.pop(uid, None)
        if txt.strip()=="📦 Files": show_files_menu(cid)
        elif txt.strip()=="📢 Force Join": show_fj_menu(cid)
        elif txt.strip()=="📣 Broadcast":
        ...
        elif txt.strip()=="👥 Users": show_users_menu(cid)
        elif txt.strip()=="⚙️ Settings": show_settings_menu(cid)
        elif txt.strip()=="🔑 Keys": _keys_admin_menu(cid)
        elif txt.strip()=="⚙️ /setup Content":
