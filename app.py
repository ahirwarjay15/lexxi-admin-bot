...
def get_unjoined(user_id):
    """Return active channels still not joined/requested by the user.

    A pending join request counts as verified immediately, even if Telegram
    still reports the user as left until the request is approved.
    """
        ...
        try:
            ...
            for row in rows:
                if str(row.get("status", "pending")).lower() == "pending":
                    pending_chat_ids.add(str(row.get("chat_id")))
        except Exception:
            pass
    ...
    for c in channels:
        ...
        # Explicitly check both representations to survive schemas where
        # chat_id is stored as text or bigint.
        pending = False
...
def show_admin_home(chat_id, mid=None):
    ...
    txt = f"🎛 **{mono('ADMIN CONTROL PANEL')}**"
    if mid:
...
def show_fj_menu(chat_id, mid=None):
    ...
    txt = f"📢 **{mono('Force Join Configuration')}:**"
    if mid:
        tg_call("editMessageText", {"chat_id": chat_id, "message_id": mid, "text": txt, "parse_mode": "Markdown", "reply_markup": {"inline_keyboard": kb}})
    else:
        tg_call("sendMessage", {"chat_id": chat_id, "text": txt, "parse_mode": "Markdown", "reply_markup": {"inline_keyboard": kb}})
        ...
        if uid in ADMIN_IDS:
            ...
            # --- Files Sub-Module ---
                ...
                if not apps:
                    ...
                    tg_call("editMessageText", {"chat_id": cid, "message_id": mid, "text": f"Select file to remove (Page {page+1}/{max(1,(total+19)//20)}):", "reply_markup": {"inline_keyboard": btns}})
                ...
                if not apps:
                    ...
                    if nav: btns.append(nav)
                    btns.append([{"text": "⬅️ "+mono("Back"), "callback_data": "m_files"}])
                    tg_call("editMessageText", {"chat_id": cid, "message_id": mid, "text": f"Select file to edit (Page {page+1}/{max(1,(total+19)//20)}):", "reply_markup": {"inline_keyboard": btns}})
            ...
            # --- Settings Sub-Module ---
                ...
                tg_call("editMessageText", {"chat_id": cid, "message_id": mid, "text": f"⚠️ **{mono('Important Notice')}**\n\nNotice: `{notice_state}`\nJOIN NOW: `{btn_state}`", "reply_markup": {"inline_keyboard": kb}})
