...
def tg_call(method, data):
    try:
        ...
        if not r.get("ok"):
            print(f"TELEGRAM API FAILED method={method} response={r}", flush=True)
        return r
...
def _zapupi_create_order(order):
    ...
    try:
        ...
        if not payment_url:
            return None, "ZapUPI did not return payment_url"
        payment_id = data.get("pay_id") or data.get("payment_id") or body.get("pay_id")
        return {"payment_url": str(payment_url), "payment_id": str(payment_id) if payment_id else None}, None
    except Exception as exc:
...
def _new_order(user_id, pid, did):
    ...
    if zap_key:
        ...
        with KEY_DATA_LOCK:
            ...
            saved["payment_url"] = payment["payment_url"]
...
def _payment_message(chat_id, order):
    link = str(order.get("payment_url") or "").strip() or _qr_link(order["amount"], order["id"], order["payment_ref"])
    rows=[]; actions={}
    if link:
...
def _handle_key_reply_text(uid,cid,txt):
    ...
    if action.startswith("PAYURL:"):
        url=action[7:].strip()
        tg_call("sendMessage",{"chat_id":cid,"text":f"💳 <a href=\"{url}\">Open Payment Page</a>","parse_mode":"HTML","disable_web_page_preview":False})
        return True
...
def process_message_update(data):
    ...
    print(f"MESSAGE RECEIVED user={uid} chat={cid} text={txt[:80]!r}", flush=True)
...
def webhook():
    ...
    # Message Router. Critical user commands are handled directly so a healthy
    # Telegram -> Render webhook cannot return 200 while the background queue
    # is saturated/stuck. Other messages remain asynchronous.
    if "message" in data:
        msg = data.get("message", {})
        uid = msg.get("from", {}).get("id")
        txt = msg.get("text", "") or ""
        cmd = txt.strip().split(maxsplit=1)[0].split("@", 1)[0].lower() if txt.strip() else ""
        if cmd in {"/start", "/buykeys", "/mykeys", "/admin"}:
            print(f"DIRECT COMMAND user={uid} cmd={cmd}", flush=True)
