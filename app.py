...
def _button_color_menu(cid, mid):
    groups = [
        ...
        ("⚙️ Settings", "settings", ["set_grp_fj","set_grp_verify","set_grp_msg","set_grp_delivery","set_grp_buttons","set_grp_system"]),
...
def tg_call(method, data):
    try:
        ...
        # Normalize every keyboard before it reaches Telegram. Admin labels keep
        # the existing monospace look; public buttons also receive real styles.
        if isinstance(data.get("reply_markup"), dict):
            ...
            if isinstance(markup0.get("inline_keyboard"), list):
                ...
                data["reply_markup"] = {
                    **markup0,
                    "inline_keyboard": mono_admin_kb(kb) if chat_id in ADMIN_IDS else _style_keyboard(kb)
                }
            elif isinstance(markup0.get("keyboard"), list):
                data["reply_markup"] = {
                    **markup0,
                    "keyboard": _style_keyboard(markup0.get("keyboard") or [])
                }

        # Keep the hot path short. A parse-mode retry is useful only for Telegram
        # 400 formatting errors; retrying network/server failures doubles latency.
        r = _TG_SESSION.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",
            json=data, timeout=(2.0, 6.0)
        ).json()
        desc = str(r.get("description", "")).lower()
        if (not r.get("ok") and "parse_mode" in data and int(r.get("error_code", 0) or 0) == 400
                and any(x in desc for x in ("parse", "markdown", "entities"))):
            retry_data = dict(data)
            retry_data.pop("parse_mode", None)
            r = _TG_SESSION.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",
                json=retry_data, timeout=(2.0, 6.0)
            ).json()
    ...
    def check_channel(c):
        ...
        try:
            res = _TG_SESSION.post(
                ...
                json={"chat_id": chat_id, "user_id": uid}, timeout=(1.5, 3.5)
            ).json()
        except Exception as exc:
        ...
        # --- Broadcast Sub-Module ---
            ...
            try:
                ...
                if mode == "all" and supabase and records:
                    try:
                        legacy_users = supabase.table("users").select("telegram_user_id").execute().data or []
            ...
            try:
                ...
                users = [u for u in users if u.get("telegram_user_id") is not None and int(u.get("telegram_user_id")) not in blocked]
                new_messages = {}
                sent = 0
                failed = 0
                telegram_blocked = 0
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if cdata.startswith("v:"):
            ...
            # The callback is already acknowledged above; queue the UI work so
            # Telegram gets the webhook HTTP 200 immediately even if Supabase or
            # Telegram is briefly slow. A dedicated pool prevents verification
            # jobs from starving admin navigation.
