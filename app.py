...
def get_unjoined(user_id):
    ...
    for c in channels:
        ...
        # If Telegram says left/kicked (or the membership lookup failed),
        # accept a pending join request as joined. Check both string/int
        # chat_id forms because Supabase column types can vary.
        pending = False
        if supabase:
            for chat_value in (str(chat_id), int(chat_id) if str(chat_id).lstrip("-").isdigit() else chat_id):
                try:
                    jr = (supabase.table("join_requests")
                          .select("user_id,chat_id")
                          .eq("user_id", int(user_id))
                          .eq("chat_id", chat_value)
                          .limit(1).execute())
                    if jr.data:
                        pending = True
                        break
...
def show_settings_menu(chat_id, mid=None):
    ...
    kb = [
        ...
        [{"text": "⚠️ Notice Msg", "callback_data": "s_warn"}],
