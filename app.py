...
def process_message_update(data):
    ...
    if txt.startswith("/start"):
        try:
            ...
            if code:
                # A deep-link must never fail silently. Run the complete user
                # flow inside this protected block so an unexpected Telegram /
                # Supabase response can be converted into a visible error.
                if get_setting("verification_enabled", "true").lower() != "true":
                ...
                tg_call("sendMessage", {
                    "chat_id": cid,
                    "text": get_setting("welcome_text", DEFAULT_WELCOME),
                    "parse_mode": "Markdown"
                })
        except Exception as exc:
            ...
            tg_call("sendMessage", {
                "chat_id": cid,
                "text": get_setting("error_feedback_text", DEFAULT_ERROR),
                "parse_mode": "Markdown"
            })
