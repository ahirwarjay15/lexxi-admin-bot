...
# Exact Stylized Typography Defaults
...
def send_or_update_force_join(chat_id, user_id, deep_link, user_name="User", message_id=None):
    ...
    if message_id:
        if has_photo:
            send_tg_request("editMessageCaption", {"chat_id": chat_id, "message_id": message_id, "caption": fj_caption, "parse_mode": "Markdown", "reply_markup": reply_markup})
        else:
            send_tg_request("editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": fj_caption, "parse_mode": "Markdown", "reply_markup": reply_markup})
    else:
        if has_photo:
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if cb_data.startswith("verify:"):
            ...
            if not get_unjoined_channels(user_id):
            ...
            if cb_data == "adm_main":
                ...
                for c in channels:
                    ...
                    btns.append([{"text": f"Slot {c['slot']} {status} {c.get('button_text')}", "callback_data": f"slot_tog_{c['slot']}"}])
