...
def tg_call(method, data):
    try:
        ...
        try:
            ...
            return {}
        if not r.get("ok"):
            print(f"TELEGRAM API FAIL method={method} http={resp.status_code} code={r.get('error_code')} desc={r.get('description')}", flush=True)
            if "parse_mode" in data:
                retry_data = dict(data); retry_data.pop("parse_mode", None)
                try:
...
def _admin_begin_input(chat_id, user_id, session, prompt):
    # Input prompts are not menus, but the previous menu must disappear before
    # an admin starts typing so there is never a stale menu behind the prompt.
    _admin_clear_menu(chat_id)
    ADMIN_SESSIONS[int(user_id)] = dict(session)
    return tg_call("sendMessage", {"chat_id": chat_id, "text": prompt})
...
def _ui_button_style(role="default", fallback="primary"):
...
def _style_inline_keyboard(kb, role_default="default"):
    ...
    for row in kb or []:
        ...
        for btn in row or []:
            ...
            if "style" not in b:
                b["style"]=_ui_button_style(role,"danger" if role=="back" else ("success" if role=="buy" else "primary"))
            nr.append(b)
        out.append(nr)
    return out
...
def show_ui_studio(chat_id,mid=None):
    ...
    txt="🎨 **UI STUDIO**\n\nChoose the native Telegram button color for each menu role.\n\n🔵 Primary • 🟢 Success • 🔴 Danger"
