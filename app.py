...
def get_unjoined(user_id):
    ...
    pending_chat_ids = {chat for (pending_uid, chat), ts in PENDING_JOIN_REQUESTS.items()
                         if pending_uid == uid}
    if supabase:
        try:
    ...
    for c in channels:
        ...
        # Normal membership check for users who actually joined.
        try:
            res = requests.post(
        ...
        # This handles rows created under either TEXT or BIGINT schemas.
        pending = False
