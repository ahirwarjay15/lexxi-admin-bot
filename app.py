...
def _redis_client():
    ...
    if not REDIS_URL or _redis_lib is None or _REDIS_DISABLED:
        return None
...
def _button_color_menu(cid, mid):
    groups = [
        ...
        ("📢 Force Join Menu", "join", ["fj_caption","fj_poster","fj_add","fj_edit","fj_onoff","fj_rem"]),
...
def tg_call(method, data):
    ...
    last = {}
    try:
        max_attempts = max(1, min(int(float(get_setting("telegram_retry_attempts", str(DEFAULT_RETRY_ATTEMPTS)))), 8))
    except Exception:
        max_attempts = DEFAULT_RETRY_ATTEMPTS
    try:
        max_delay = max(0.5, min(float(get_setting("telegram_retry_max_delay", str(DEFAULT_RETRY_MAX_DELAY))), 30.0))
    except Exception:
        max_delay = DEFAULT_RETRY_MAX_DELAY
    for attempt in range(max_attempts):
        try:
            resp = _TG_SESSION.post(f"https://api.telegram.org/bot{BOT_TOKEN}/{method}", json=data, timeout=(1.5, 5.0))
            try: r = resp.json()
            except Exception: r = {}
            last = r
            if r.get("ok") or not _telegram_retryable(r):
                desc = str(r.get("description", "")).lower()
                if (not r.get("ok") and "parse_mode" in data and int(r.get("error_code",0) or 0)==400 and any(x in desc for x in ("parse","markdown","entities"))):
                    retry_data = dict(data); retry_data.pop("parse_mode", None)
                    try: return _TG_SESSION.post(f"https://api.telegram.org/bot{BOT_TOKEN}/{method}", json=retry_data, timeout=(1.5,5.0)).json()
    ...
    def check_channel(c):
        ...
        if not res.get("ok"):
            ...
            if code==400 and any(x in desc for x in ("chat not found","user not found","bot is not a member","not enough rights","have no rights")):
                print(f"FORCE JOIN CHANNEL SKIPPED chat={normalized_chat_id}: {res.get('description')}",flush=True)
                return None
    ...
    while True:
        ...
        try:
            rows=(supabase.table(BACKGROUND_JOBS_TABLE).select("*").eq("status","pending")
                  .lte("run_at",time.time()).order("run_at").limit(25).execute().data or [])
        except Exception:
            continue
        for job in rows:
            jid=job.get("job_id")
            ...
            try:
                ...
                supabase.table(BACKGROUND_JOBS_TABLE).update({"status":"done" if ok else "failed","updated_at":time.time()}).eq("job_id",jid).execute()
            except Exception as exc:
...
def _broadcast_delivery_claim(broadcast_id, user_id):
    """Atomically claim one user for one broadcast.

    A UNIQUE(broadcast_id, user_id) constraint in Supabase makes this safe
    across multiple bot workers/processes. We fail closed if the delivery
    table cannot be reached, because sending without a durable claim could
    create duplicate deliveries.
    Returns: (claimed: bool, error: str|None)
    """
    if not supabase:
    ...
    try:
        ...
        msg = str(exc)
        # Duplicate unique-key means another worker/process already claimed
        # this exact broadcast + user. That user must not receive it again.
        low = msg.lower()
...
def _finish_broadcast_album(uid, cid, group_id):
    ...
    if r:
        try:
            raw=r.get(f"lexi:album:{uid}")
            if raw: item=json.loads(raw); r.delete(f"lexi:album:{uid}")
        except Exception: pass
    if item is None:
...
def settings_submenu(cid,mid,kind):
    ...
    if kind=="verify":
        txt=f"✅ **VERIFICATION**\n\nStatus: `{status('verification_enabled')}`\nMethod: `JOINED` — actual membership required\n\nJoin Request alone does not complete Force Join."
    ...
    if uid in ADMIN_IDS:
        ...
        if cdata.startswith("pickcolor:"):
            ...
            if key.startswith("fj_slot_"): _fj_color_items(cid,mid)
                ...
                maps={"menu":["adm_home","m_files","m_fj","m_bc","m_users","m_sett","m_stats"],"files":["f_add","f_del","f_edit","f_list"],"join":["fj_caption","fj_poster","fj_add","fj_edit","fj_onoff","fj_rem"],"settings":["set_grp_fj","set_grp_verify","set_grp_msg","set_grp_delivery","set_grp_buttons","set_grp_system","set_grp_reliability"],"actions":["msg_retry","s_wel","s_err","s_warn","s_timer","s_upbtn","s_layout","s_anim"]}
                    ...
                    try:
                        ...
                        if len(message_ids) > 1:
                            ...
                            if r.get("ok"):
                                ...
                                # copyMessages skips unsupported/missing messages. If it did not
                                # return the full album, retry one-by-one to avoid partial delivery.
                                if len(copied) != len(message_ids):
                                    # copyMessages returns results in request order; only retry the
                                    # missing tail so already-copied items are never duplicated.
                                    source_ids = sorted(message_ids)
                                    missing_ids = source_ids[len(copied):]
                                    for source_mid in missing_ids:
        ...
        # --- Settings Sub-Module ---
            ...
            tg_call("editMessageText",{"chat_id":cid,"message_id":mid,"text":f"❤️ **HEALTH**\n\nTelegram Token: `{tg}`\nSupabase: `{sc}`\nChannels Cache: `{cc}`\nExecutors: `READY`\nWebhook: `ACTIVE`","parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb([[{"text":"⬅️ Back","callback_data":"set_grp_reliability"}]])}})
...
def _welcome_selected_channels():
    ...
    chs=get_channels() or []
    all_ids={str(c.get("chat_id")) for c in chs if c.get("chat_id") is not None}
    raw=get_setting(WELCOME_CHANNELS_KEY, "__ALL__")
    if raw == "__ALL__" or raw == "": return all_ids
    try:
        val=json.loads(raw)
        return {str(x) for x in val} if isinstance(val,list) else all_ids
    except Exception:
        return all_ids
...
def _welcome_text(record):
    ...
    raw = get_setting("welcome_dm_text", DEFAULT_WELCOME_DM)
    return str(raw).replace("{name}", name).replace("{username}", f"@{username}" if username else "").replace("{channel_name}", channel)
...
def _welcome_send(record, force_chat_id=None):
    ...
    text = _welcome_text(record)
    markup = _welcome_keyboard()
    photo = get_setting("welcome_dm_photo", "").strip()
    payload_base = {"chat_id": int(chat_id), "parse_mode": "Markdown"}
    if photo:
...
def _welcome_handle_join_approved(cmu):
    ...
    became_member = new_status in {"member","administrator","creator","restricted"} and old_status in {"left","kicked",""}
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if cdata.startswith("v:"):
            ...
            # Long-running broadcast jobs get their own executor so a large
            # broadcast cannot starve normal admin callbacks/navigation.
            if cdata == "send_confirmed_bc":
    ...
    # Message Router: always enqueue and return immediately. This keeps the
    # webhook path free from Telegram/Supabase latency, including admin /start.
    if "message" in data:
...
def process_message_update(data):
    ...
    if uid in ADMIN_IDS and uid in ADMIN_SESSIONS:
            ...
            if media_group_id:
                ...
                if r:
                    ...
                    try:
                        ...
                        first=bool(r.set(f"lexi:album:timer:{uid}","1",nx=True,ex=15))
                    except Exception: pass
                else:
                    with BROADCAST_ALBUM_LOCK:
                        ...
                        if item.get("group_id") != str(media_group_id):
                            item = {"group_id": str(media_group_id), "message_ids": []}; BROADCAST_ALBUM_BUFFERS[uid] = item
                        if mid not in item["message_ids"]: item["message_ids"].append(mid)
        ...
        # Welcome DM settings
            ...
            set_setting("welcome_dm_text",txt); ADMIN_SESSIONS.pop(uid,None); tg_call("sendMessage",{"chat_id":cid,"text":"✅ Welcome DM updated."}); show_settings_menu(cid)
