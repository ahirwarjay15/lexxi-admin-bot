...
def tg_call(method, data):
    try:
        ...
        # =========================================================================
        # 🔥 CRITICAL FIX: SANITIZE KEYBOARD 🔥
        # Telegram API strictly rejects any payload containing unknown properties.
        # The custom 'style' attribute causes a silent 400 Bad Request error.
        # We strip it out cleanly before making the HTTP POST to Telegram.
        # =========================================================================
                ...
                for row in payload["reply_markup"]["inline_keyboard"]:
                    ...
                    for btn in row:
                        if isinstance(btn, dict):
                            c_btn = dict(btn)
                            c_btn.pop("style", None)  # Remove the invalid field
                            clean_row.append(c_btn)
                        else:
                            clean_row.append(btn)
                    clean_kb.append(clean_row)
                payload["reply_markup"]["inline_keyboard"] = clean_kb
