...
def tg_call(method, data):
    try:
        ...
        if r.get("ok") and method in {"sendMessage", "copyMessage", "forwardMessage", "sendPhoto", "sendVideo", "sendDocument", "sendAnimation", "sendAudio", "sendVoice", "sendSticker"}:
            ...
            if mid:
                _track_chat_message(data.get("chat_id"), mid)
        return r
...
def process_message_update(data):
    ...
    # Admin Reply Keyboard navigation.  Do this only when no multi-step input
    # session is active, so typed content for an edit is never swallowed.
    admin_button_routes = {
        "📦 Files": "m_files",
        "📢 Force Join": "m_fj",
        "📣 Broadcast": "m_bc",
        ...
        "⚙️ Settings": "m_sett",
        "📊 Analytics": "m_stats",
        "🧹 Cleanup": "set_grp_cleanup",
        "✨ Message Studio": "msg_studio",
        "🛠 System": "set_system",
    }
    if uid in ADMIN_IDS and not ADMIN_SESSIONS and txt.strip() in admin_button_routes:
        action = admin_button_routes[txt.strip()]
        if action == "m_files":
        ...
        if action == "m_bc":
            ADMIN_SESSIONS[uid] = {"step": "WAIT_BC_CONTENT"}
            tg_call("sendMessage", {"chat_id":cid,"text":"📣 **BROADCAST**\n\nSend or forward the message to broadcast.\n\n🧹 Before each send, tracked user/bot messages and the previous broadcast are cleared.\n🚫 Broadcast messages have no auto-delete timer.","parse_mode":"Markdown"})
            return "OK", 200
        if action == "m_users":
            show_users_menu(cid); return "OK", 200
        if action == "m_sett":
            show_settings_menu(cid); return "OK", 200
        if action == "m_stats":
            _show_admin_stats(cid); return "OK", 200
        if action == "set_grp_cleanup":
            # Keep detailed toggles in an inline control sheet.
            tg_call("sendMessage", {"chat_id":cid,"text":"🧹 **USER MESSAGE CLEANUP**\n\nTemporary user-flow messages use their own independent TTL. File expiry remains separate.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🧹 Open Cleanup Controls","callback_data":"set_grp_cleanup"}],[{"text":"🏠 Admin Home","callback_data":"adm_home"}]]}})
            return "OK", 200
        if action == "msg_studio":
            _show_admin_studio(cid); return "OK", 200
        if action == "set_system":
            tg_call("sendMessage", {"chat_id":cid,"text":"🛠 **SYSTEM**\n\n⚡ Webhook-first processing\n⚡ Parallel channel verification\n⚡ Cached settings and channels\n⚡ Dynamic channel count","parse_mode":"Markdown","reply_markup":{"inline_keyboard":[[{"text":"🏠 Admin Home","callback_data":"adm_home"}]]}})
            return "OK", 200
    if uid in ADMIN_IDS and txt.strip() == "🛠 Admin Panel" and not ADMIN_SESSIONS:
        show_admin_home(cid)
        return "OK", 200
