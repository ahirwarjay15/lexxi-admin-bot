...
def _button_color(callback, label=None, default=None):
    ...
    if key not in BUTTON_COLORS:
        ...
        # Never perform a blocking Supabase read while rendering an Admin keyboard.
        # The old lazy DB read could hold the settings lock during a slow Supabase
        # request and let Gunicorn kill the worker while an admin opened the panel.
        saved = _SETTINGS_CACHE.get("btncolor_" + key, "") if _SETTINGS_CACHE_READY else ""
