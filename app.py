...
VERIFICATION_LOCKS = {}
# Public user-flow message cleanup is intentionally separate from file expiry.
PUBLIC_MESSAGE_DELETE_TIMERS = {}
PUBLIC_MESSAGE_DELETE_LOCK = threading.Lock()
# Per-user chat history used for pre-broadcast cleanup. Telegram has no API to
# enumerate arbitrary private-chat history, so messages are tracked as updates
# arrive and every bot-sent message is registered by tg_call().
CHAT_MESSAGE_HISTORY = {}
CHAT_MESSAGE_HISTORY_LOCK = threading.Lock()
BROADCAST_MESSAGE_IDS = {}
# Per-admin reply-keyboard routing. Telegram reply-keyboard buttons arrive as
# ordinary text messages, so we remember the callback represented by each
# visible button for the current admin screen.
ADMIN_REPLY_ROUTES = {}
ADMIN_REPLY_KEYBOARDS = {}
ADMIN_REPLY_LOCK = threading.Lock()
# Only one Admin Panel/menu message is kept in the chat. Every time a new
# admin menu is opened, the previous menu message(s) are removed.
ADMIN_MENU_MESSAGES = {}
ADMIN_MENU_LOCK = threading.Lock()
...
DEFAULT_VERIFY_BUTTON = "» VERIFY"
DEFAULT_USER_MSG_TIMER = "120"
...
# Telegram Reply Keyboard for the admin workspace.  The main admin navigation
# lives in the keyboard area (like a native bot control bar); detailed actions
# may still use inline keyboards where dynamic lists/confirmations are needed.
def _admin_reply_keyboard():
    return {
        "keyboard": [
            [{"text": "📦 Files"}, {"text": "🛠 Admin Panel"}],
            [{"text": "📢 Force Join"}, {"text": "📣 Broadcast"}],
            [{"text": "👥 Users"}, {"text": "⚙️ Settings"}],
            [{"text": "📊 Analytics"}, {"text": "🧹 Cleanup"}],
            [{"text": "✨ Message Studio"}, {"text": "🛠 System"}],
            [{"text": "⌨️ Hide Admin Panel"}]
        ],
...
def _show_admin_reply_keyboard(chat_id):
    return tg_call("sendMessage", {
        "chat_id": chat_id,
        "text": "🛠 **ADMIN CONTROL BAR**\n\nUse the keyboard below to navigate the Admin Panel.\nYour normal text input remains available for every edit/input step.",
        "parse_mode": "Markdown",
        "reply_markup": _admin_reply_keyboard()
    })

def _hide_admin_reply_keyboard(chat_id):
    _delete_admin_menu_ids(chat_id)
    return tg_call("sendMessage", {
        "chat_id": chat_id,
        "text": "⌨️ Admin keyboard hidden. Send /admin anytime to reopen it.",
        "reply_markup": {"remove_keyboard": True}
    })

def _show_admin_stats(chat_id):
    ...
    active=len([c for c in channels if c.get("active")])
    cleanup_on=get_setting("user_message_cleanup_enabled", "true").lower() == "true"
    msg_timer=get_setting("user_message_cleanup_time", DEFAULT_USER_MSG_TIMER)
    file_timer=get_setting("auto_delete_time", "120")
    txt=(f"📊 **SYSTEM OVERVIEW**\n\n"
         f"👥 Users: `{users}`\n"
         f"📦 Files: `{files}`\n"
         f"📢 Active FJ Channels: `{active}`\n\n"
         f"🧹 User-message cleanup: `{'ON' if cleanup_on else 'OFF'}`\n"
         f"⏱ User-message timer: `{msg_timer}s`\n"
         f"📦 File expiry timer: `{file_timer}s`\n\n"
         "🟢 Core services ready")
    return tg_call("sendMessage", {"chat_id": chat_id, "text": txt, "parse_mode": "Markdown",
        "reply_markup": {"inline_keyboard": [[{"text":"🔄 Refresh","callback_data":"m_stats"}], [{"text":"🏠 Admin Home","callback_data":"adm_home"}]]}})

def _show_admin_studio(chat_id):
    kb=[]
    for label,key in _admin_editor_items():
        kb.append([{"text": label, "callback_data": f"editadmin_{key}"}])
    kb.append([{"text": "ℹ️ Custom/Premium emojis + formatting are preserved", "callback_data": "noop"}])
    kb.append([{"text": "🏠 Admin Home", "callback_data": "adm_home"}])
    return tg_call("sendMessage", {"chat_id":chat_id,
        "text":"🛠 **ADMIN PANEL TEXT STUDIO**\n\nCustomize the Admin Panel screen text.\n\n💎 Send your replacement as a normal Telegram message; custom/premium emoji entities, formatting and links are preserved.",
...
def _track_chat_message(chat_id, message_id):
    ...
    try:
        ...
        key = str(chat_id)
    with CHAT_MESSAGE_HISTORY_LOCK:
        bucket = CHAT_MESSAGE_HISTORY.setdefault(key, [])
        if message_id not in bucket:
            bucket.append(int(message_id))
        # Prevent unbounded memory use while keeping a practical chat history.
        if len(bucket) > 500:
            del bucket[:-500]

def _clear_chat_before_broadcast(chat_id):
    """Best-effort clear of tracked user/bot messages plus the last persisted broadcast."""
    try:
        key = int(chat_id)
    except Exception:
        key = str(chat_id)
    with CHAT_MESSAGE_HISTORY_LOCK:
        mids = list(CHAT_MESSAGE_HISTORY.get(key, []))
        CHAT_MESSAGE_HISTORY[key] = []
    # Persisted broadcast IDs survive bot restarts, so an old broadcast can
    # still be removed before the next one is delivered.
    try:
        saved = json.loads(get_setting("broadcast_message_ids", "{}") or "{}")
        for mid in saved.get(str(key), []) if isinstance(saved, dict) else []:
            if mid not in mids:
                mids.append(int(mid))
        if isinstance(saved, dict):
            saved.pop(str(key), None)
            set_setting("broadcast_message_ids", json.dumps(saved))
    except Exception:
        pass
    for message_id in reversed(list(dict.fromkeys(mids))):
        try:
            tg_call("deleteMessage", {"chat_id": chat_id, "message_id": message_id})
        except Exception:
            pass

def _remember_broadcast(chat_id, message_id):
    if not message_id:
        return
    try:
        key = str(int(chat_id))
    ...
    try:
        ...
        if not isinstance(saved, dict):
            saved = {}
        saved[key] = [int(message_id)]
        set_setting("broadcast_message_ids", json.dumps(saved))
    except Exception:
        pass

def _message_entities(msg):
    """Return Telegram entities from an admin-sent message, including premium custom emoji."""
    entities = msg.get("entities") or []
    return entities if isinstance(entities, list) else []

def _set_message_template(key, msg):
    """Persist text plus Telegram entities so custom/premium emojis survive later sends."""
    payload = {"text": msg.get("text", "") or "", "entities": _message_entities(msg)}
    set_setting(key, json.dumps(payload, ensure_ascii=False))

def _get_message_template(key, default):
    raw = get_setting(key, default)
    try:
        obj = json.loads(raw) if isinstance(raw, str) and raw.startswith("{") else None
        if isinstance(obj, dict) and "text" in obj:
            return obj.get("text", ""), obj.get("entities") or []
    except Exception:
        pass
    return str(raw or ""), []
...
def _template_payload(key, default, replacements=None):
    text, entities = _get_message_template(key, default)
    ...
    for old, new in replacements.items():
        ...
        if pos < 0:
            continue
        old_u16 = _utf16_len(old)
        new_u16 = _utf16_len(new)
        start_u16 = _utf16_len(text[:pos])
        ...
        for ent in entities:
            ...
            if e_end <= start_u16:
                ...
                e["offset"] = off + (new_u16 - old_u16)
            elif off >= start_u16 and e_end <= end_u16:
                # Entity wholly inside a replaced placeholder cannot safely be
                # preserved; drop it rather than sending invalid offsets.
                continue
            else:
                # Overlapping the placeholder: preserve the portion after it.
                if off < start_u16:
                    e["length"] = max(0, start_u16 - off)
                else:
                    e["offset"] = start_u16 + new_u16
                    e["length"] = max(0, e_end - end_u16)
            if e.get("length", 0) > 0:
                updated.append(e)
...
ADMIN_UI_TEMPLATES = {
    "admin_home_text": "",
    "admin_files_text": "✦ **FILE VAULT** ✦\n\nManage, publish, edit and retire your stored file links.",
    "admin_fj_text": "✦ **FORCE JOIN CONTROL** ✦\n\nConfigure channels, presentation and verification flow.",
    "admin_settings_text": "✦ **SETTINGS HUB** ✦\n\nEverything is grouped into clean modules.\nChanges apply immediately and are stored persistently.",
    "admin_users_text": "👥 **Users Overview:**\n\n• Total Active Users: `{count}`",
}

def _admin_template_payload(key, default, replacements=None):
    return _template_payload(key, default, replacements or {})

def _admin_screen_call(method, chat_id, mid, key, default, reply_markup=None, replacements=None):
    text, entities = _admin_template_payload(key, default, replacements)
    ...
    return tg_call(method, payload)

def _admin_editor_items():
    return [
        ("🏠 Admin Home", "admin_home_text"),
        ("📦 Files", "admin_files_text"),
        ("📢 Force Join", "admin_fj_text"),
        ("⚙️ Settings Hub", "admin_settings_text"),
        ("👥 Users", "admin_users_text"),
    ]

def _admin_reply_markup_from_inline(chat_id, inline_markup):
    """Convert an Admin Panel inline keyboard into a native Telegram reply keyboard.

    All admin-panel controls live in the keyboard area. The message above is
    only the information/details. Button presses come back as normal text and
    are routed to their original callback_data.
    """
    rows = (inline_markup or {}).get("inline_keyboard") or []
    keyboard = []
    routes = {}
    for row in rows:
        out_row = []
        for btn in row or []:
            ...
            if not label:
                continue
            # Telegram reply keyboards cannot carry callback_data. Store the
            # mapping server-side and show only the clean button label.
            out_row.append({"text": label})
            if callback is not None:
                routes[label] = str(callback)
        if out_row:
    ...
    with ADMIN_REPLY_LOCK:
        ADMIN_REPLY_ROUTES[str(chat_id)] = routes
        ADMIN_REPLY_KEYBOARDS[str(chat_id)] = markup
    return markup


def _admin_route_text(chat_id, text):
    with ADMIN_REPLY_LOCK:
        return (ADMIN_REPLY_ROUTES.get(str(chat_id)) or {}).get(str(text).strip())


def _admin_current_keyboard(chat_id):
    with ADMIN_REPLY_LOCK:
        return ADMIN_REPLY_KEYBOARDS.get(str(chat_id))


def _admin_menu_ids(chat_id):
    with ADMIN_MENU_LOCK:
        return list(ADMIN_MENU_MESSAGES.get(str(chat_id), []))


def _register_admin_menu(chat_id, message_id):
    if chat_id is None or not message_id:
        return
    with ADMIN_MENU_LOCK:
        ADMIN_MENU_MESSAGES[str(chat_id)] = [int(message_id)]


def _delete_admin_menu_ids(chat_id, ids=None, keep=None):
    ids = _admin_menu_ids(chat_id) if ids is None else list(ids)
    if keep is not None:
        ids = [m for m in ids if int(m) != int(keep)]
    for old_mid in ids:
        try:
            _TG_SESSION.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage",
                json={"chat_id": chat_id, "message_id": int(old_mid)}, timeout=(2.5, 8)
            )
        except Exception:
            pass
    with ADMIN_MENU_LOCK:
        current = ADMIN_MENU_MESSAGES.get(str(chat_id), [])
        if keep is not None:
            ADMIN_MENU_MESSAGES[str(chat_id)] = [int(keep)]
        else:
            ADMIN_MENU_MESSAGES.pop(str(chat_id), None)


def _admin_menu_send_cleanup(chat_id, message_id, old_ids):
    # Keep exactly one menu message. Do this only after the replacement was
    # successfully sent so a failed Telegram request never leaves the admin
    # without a usable menu.
    if message_id:
        _register_admin_menu(chat_id, message_id)
        for old_mid in old_ids:
            if int(old_mid) == int(message_id):
                continue
            try:
                _TG_SESSION.post(
                    f"https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage",
...
def tg_call(method, data):
    try:
        # Admin Panel UI: replace every inline admin keyboard with a native
        # bottom reply keyboard. Dynamic buttons (files, slots, confirmations,
        # settings, etc.) are routed back to their original callback_data.
        payload = dict(data or {})
        chat_id = payload.get("chat_id")
        # Internal-only marker: never send this field to Telegram. It is useful
        # for the native ReplyKeyboard admin UI, whose menu message cannot be
        # edited in-place.
        explicit_admin_menu = bool(payload.pop("__admin_menu__", False))
        is_admin_chat = False
        try:
            is_admin_chat = int(chat_id) in ADMIN_IDS
        except Exception:
            pass

        admin_menu_request = False
        old_menu_ids = []
        if is_admin_chat and isinstance(payload.get("reply_markup"), dict) and payload["reply_markup"].get("inline_keyboard"):
            inline = payload["reply_markup"]
            payload["reply_markup"] = _admin_reply_markup_from_inline(chat_id, inline)
            admin_menu_request = True
            old_menu_ids = _admin_menu_ids(chat_id)
            if method == "editMessageText" and payload.get("message_id"):
                # ReplyKeyboardMarkup cannot be attached to editMessageText.
                # Replace the old screen with one fresh menu message. The old
                # message is deleted only after the replacement succeeds.
                requested_mid = payload.pop("message_id")
                if requested_mid not in old_menu_ids:
                    old_menu_ids.append(requested_mid)
                method = "sendMessage"
        elif is_admin_chat and explicit_admin_menu:
            admin_menu_request = True
            old_menu_ids = _admin_menu_ids(chat_id)

        # Keep the current Admin Panel keyboard visible on ordinary detail /
        # prompt messages too, unless the caller explicitly supplied markup.
        if is_admin_chat and method in {"sendMessage", "sendPhoto", "sendVideo", "sendDocument"} and "reply_markup" not in payload:
            current = _admin_current_keyboard(chat_id)
            if current:
                payload["reply_markup"] = current

        r = _TG_SESSION.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",
            json=payload, timeout=(2.5, 8)
        ).json()
        if not r.get("ok") and "parse_mode" in payload:
            retry_data = dict(payload)
            retry_data.pop("parse_mode", None)
            r = _TG_SESSION.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",
                json=retry_data, timeout=(2.5, 8)
            ).json()
        if r.get("ok") and method in {"sendMessage", "copyMessage", "forwardMessage", "sendPhoto", "sendVideo", "sendDocument", "sendAnimation", "sendAudio", "sendVoice", "sendSticker"}:
            mid = (r.get("result") or {}).get("message_id")
            if mid:
                _track_chat_message(payload.get("chat_id"), mid)
                if admin_menu_request:
                    _admin_menu_send_cleanup(payload.get("chat_id"), mid, old_menu_ids)
        return r
...
def _fj_keyboard(unjoined, code):
    ...
    verify_btn = get_setting("verify_button_text", DEFAULT_VERIFY_BUTTON) or DEFAULT_VERIFY_BUTTON
    kb.append([{"text": verify_btn, "callback_data": f"v:{code}"}])
    return kb
...
def _schedule_public_message_delete(chat_id, message_id, delay=None):
    """Delete a bot-sent public-flow message after its own TTL.

    This timer is deliberately independent from the file expiry timer.
    """
    if not message_id:
        return
    if get_setting("user_message_cleanup_enabled", "true").lower() != "true":
        return
    try:
        delay = int(delay if delay is not None else get_setting("user_message_cleanup_time", DEFAULT_USER_MSG_TIMER))
    except (TypeError, ValueError):
        delay = 120
    ...
    def worker():
        time.sleep(delay)
        tg_call("deleteMessage", {"chat_id": chat_id, "message_id": message_id})
        with PUBLIC_MESSAGE_DELETE_LOCK:
            PUBLIC_MESSAGE_DELETE_TIMERS.pop((chat_id, message_id), None)

    key = (chat_id, message_id)
    with PUBLIC_MESSAGE_DELETE_LOCK:
        if key in PUBLIC_MESSAGE_DELETE_TIMERS:
            return
        t = threading.Thread(target=worker, name="public-message-cleanup", daemon=True)
        PUBLIC_MESSAGE_DELETE_TIMERS[key] = t
        t.start()

def _track_public_result(chat_id, result, category="flow"):
    """Track a successful bot message/copyMessage for public-flow cleanup."""
    if not isinstance(result, dict) or not result.get("ok"):
        return result
    mid = result.get("result", {}).get("message_id")
    if not mid:
        return result
    key = f"cleanup_{category}"
    if get_setting(key, "true").lower() == "true":
        _schedule_public_message_delete(chat_id, mid)
    return result
...
def _send_template_message(chat_id, key, default, replacements=None, reply_markup=None, category="flow"):
    text, entities = _template_payload(key, default, replacements or {})
    payload = {"chat_id": chat_id, "text": text}
    if entities:
        payload["entities"] = entities
    else:
    ...
    if result.get("ok") and get_setting("user_message_cleanup_enabled", "true").lower() == "true":
        if category == "error" or get_setting("cleanup_flow", "true").lower() == "true":
            _schedule_public_message_delete(chat_id, result["result"].get("message_id"))
    return result
...
def send_fj(chat_id, user_id, code, name):
    ...
    if not unjoined:
        return True
    caption, caption_entities = _template_payload("force_join_text", DEFAULT_FJ, {"{name}": clean_markdown(name)})
    ...
    if poster_storage_id:
        payload = {"chat_id": chat_id, "from_chat_id": storage_id, "message_id": int(poster_storage_id), "caption": caption, "reply_markup": {"inline_keyboard": kb}}
        if caption_entities:
        ...
        if caption_entities:
            payload["entities"] = caption_entities
        else:
            payload["parse_mode"] = "Markdown"
        res=tg_call("sendMessage", payload)
    if res.get("ok"):
        _track_verification(chat_id,user_id,code,base=res["result"]["message_id"])
        if get_setting("cleanup_fj", "true").lower() == "true":
            _schedule_public_message_delete(chat_id, res["result"]["message_id"])
    return False
...
def deliver_file(chat_id, code):
    ...
    if not app_data:
        _track_public_result(chat_id, _send_template_message(chat_id, "error_feedback_text", DEFAULT_ERROR, category="error"), "error")
        return
    ...
    if del_seconds > 0 and (cleanup_file or cleanup_notice):
        ...
        if not notice_enabled:
            ...
            warn_msg, warn_entities = _template_payload("auto_delete_warn_text", DEFAULT_WARN, {"{time}": time_str})
        ...
        if warn_msg and cleanup_notice:
            w = tg_call("sendMessage", {
                "chat_id": chat_id,
                "text": warn_msg,
                **({"entities": warn_entities} if warn_entities else {"parse_mode": "Markdown"}),
...
def run_verify_process(chat_id, user_id, code, fj_mid):
    ...
    try:
        if get_setting("verification_enabled", "true").lower() != "true":
            cleanup_verification_state(chat_id, user_id, code, include_base=False)
        ...
        try:
            ...
            anim_speed = 0.50
        base, base_entities = _get_message_template("anim_text", DEFAULT_ANIM_TEXT)
        base = str(base).strip() or "Verifying"
        ...
        def animate():
            ...
            if base_entities:
                anim_payload["entities"] = base_entities
            r = tg_call("sendMessage", anim_payload)
            if r.get("ok") and get_setting("cleanup_animation", "true").lower() == "true":
                _schedule_public_message_delete(chat_id, r["result"].get("message_id"))
            ...
            while not stop.wait(anim_speed):
                ...
                if base_entities:
                    anim_edit["entities"] = base_entities
        ...
        if anim_mid:
            # IMPORTANT: verification animation must ALWAYS be removed first.
            # Its cleanup timer is only a fallback; it must never delay the
            # next user-facing step (file delivery or retry/FJ message).
        ...
        retry_text, retry_entities = _template_payload("verify_retry_text", DEFAULT_VERIFY_RETRY)
        retry_payload = {"chat_id": chat_id, "text": retry_text}
        if retry_entities:
            retry_payload["entities"] = retry_entities
        else:
            retry_payload["parse_mode"] = "Markdown"
        r = tg_call("sendMessage", retry_payload)
        if r.get("ok") and get_setting("cleanup_retry", "true").lower() == "true":
            _schedule_public_message_delete(chat_id, r["result"].get("message_id"))
        if r.get("ok"):
...
def show_admin_home(chat_id, mid=None):
    ...
    channels = get_channels() or []
    active = len([c for c in channels if c.get("active")])
    msg_cleanup = get_setting("user_message_cleanup_enabled", "true").lower() == "true"
    msg_timer = get_setting("user_message_cleanup_time", DEFAULT_USER_MSG_TIMER)
    file_timer = get_setting("auto_delete_time", "120")
    default_txt=("✦ **LEXI CONTROL CENTER** ✦\n\n"
         "🟢 **SYSTEM ONLINE**  •  Webhook Ready\n\n"
         f"👥 Users     `{users}`     •     📦 Files     `{files}`\n"
         f"📢 FJ Active `{active}`     •     🧹 Cleanup   `{'ON' if msg_cleanup else 'OFF'}`\n\n"
         f"⏱ Message TTL  `{msg_timer}s`   |   📦 File TTL  `{file_timer}s`\n\n"
         "Use the keyboard below to control everything.\n"
         "💎 Text/emoji edits are entered normally from the keyboard.")
    txt, entities = _admin_template_payload("admin_home_text", default_txt, {"{users}": str(users), "{files}": str(files), "{active}": str(active), "{cleanup}": "ON" if msg_cleanup else "OFF", "{msg_timer}": str(msg_timer), "{file_timer}": str(file_timer)})

    # ReplyKeyboard cannot be attached to editMessageText.  When navigating
    # back to Home, replace the old panel message and expose the native
    # keyboard-area admin workspace.
    if mid:
        try:
            tg_call("deleteMessage", {"chat_id": chat_id, "message_id": mid})
        except Exception:
            pass
    data={"chat_id":chat_id,"text":txt,"reply_markup":_admin_reply_keyboard(),"__admin_menu__":True}
    if entities:
...
def show_files_menu(chat_id, mid=None):
    ...
    txt, entities = _admin_template_payload("admin_files_text", "✦ **FILE VAULT** ✦\n\nManage, publish, edit and retire your stored file links.")
    data = {"chat_id": chat_id, "text": txt, "reply_markup": {"inline_keyboard": kb}}
    if mid: data["message_id"] = mid
    if entities: data["entities"] = entities
    else: data["parse_mode"] = "Markdown"
...
def show_fj_menu(chat_id, mid=None):
    ...
    txt, entities = _admin_template_payload("admin_fj_text", "✦ **FORCE JOIN CONTROL** ✦\n\nConfigure channels, presentation and verification flow.")
    data = {"chat_id": chat_id, "text": txt, "reply_markup": {"inline_keyboard": kb}}
    if mid: data["message_id"] = mid
    if entities: data["entities"] = entities
    else: data["parse_mode"] = "Markdown"
...
def show_settings_menu(chat_id, mid=None):
    default_txt="✦ **SETTINGS HUB** ✦\n\nEverything is grouped into clean modules.\nChanges apply immediately and are stored persistently."
    txt, entities = _admin_template_payload("admin_settings_text", default_txt)
    kb=[
        ...
        [_settings_button("🧹 User Message Cleanup","set_grp_cleanup","mono")],
        [_settings_button("🎞 Animation & System","set_grp_system","serif")],
        [_settings_button("⬅️ Back","adm_home","mono")]
    ]
    data={"chat_id":chat_id,"message_id":mid,"text":txt,"reply_markup":{"inline_keyboard":kb}} if mid else {"chat_id":chat_id,"text":txt,"reply_markup":{"inline_keyboard":kb}}
    if entities: data["entities"] = entities
    else: data["parse_mode"] = "Markdown"
    tg_call("editMessageText" if mid else "sendMessage",data)
...
def show_settings_group(cid, mid, group):
    fonts = {
        ...
        "cleanup": ("🧹 USER MESSAGE CLEANUP", "Automatically remove bot-sent user-flow messages on their own timer."),
    ...
    if group == "fj":
        ...
        kb=[[{"text":"✨ "+mono("Message Studio"),"callback_data":"msg_studio"}],
            [{"text":"👋 "+mono("Welcome"),"callback_data":"s_wel"},{"text":"⚠️ "+mono("Error"),"callback_data":"s_err"}],
            [{"text":"🔄 "+mono("Retry"),"callback_data":"msg_retry"}],
            [{"text":"📢 "+mono("Force Join Caption"),"callback_data":"fj_caption"}],
            [{"text":"⚠️ "+mono("Important Notice"),"callback_data":"s_warn"}],
            [{"text":"🎞 "+mono("Animation Text"),"callback_data":"anim_text"}],
            [{"text":"✅ "+mono("Verify Button Label"),"callback_data":"verify_btn_edit"}]]
    elif group == "delivery":
        ...
        enabled = get_setting("user_message_cleanup_enabled", "true").lower() == "true"
        timer = get_setting("user_message_cleanup_time", DEFAULT_USER_MSG_TIMER)
        kb=[[{"text":f"🧹 Cleanup: {'🟢 ON' if enabled else '🔴 OFF'}","callback_data":"user_cleanup_toggle"}],
            [{"text":"⏱ "+mono("Message Timer"),"callback_data":"user_cleanup_timer"}],
            [{"text":"📢 "+mono("FJ Post")+f": {'🟢 ON' if get_setting('cleanup_fj','true').lower()=='true' else '🔴 OFF'}","callback_data":"toggle_cleanup_fj"}],
            [{"text":"🔄 "+mono("Retry")+f": {'🟢 ON' if get_setting('cleanup_retry','true').lower()=='true' else '🔴 OFF'}","callback_data":"toggle_cleanup_retry"}],
            [{"text":"🎞 "+mono("Animation")+f": {'🟢 ON' if get_setting('cleanup_animation','true').lower()=='true' else '🔴 OFF'}","callback_data":"toggle_cleanup_animation"}],
            [{"text":"👋 "+mono("Welcome/Error")+f": {'🟢 ON' if get_setting('cleanup_flow','true').lower()=='true' else '🔴 OFF'}","callback_data":"toggle_cleanup_flow"}]]
    else:
...
def settings_submenu(cid,mid,kind):
    ...
    if kind=="verify":
        ...
        txt="📝 **MESSAGES**\n\nEdit user-facing text directly from Admin Panel.\n\n✨ **Premium Emoji:** send the replacement as a Telegram message with your custom/premium emojis; formatting and custom emoji entities are saved and reused."
        kb=[[{"text":"✨ "+mono("Message Studio"),"callback_data":"msg_studio"}],
            [{"text":"👋 "+mono("Welcome"),"callback_data":"s_wel"},{"text":"⚠️ "+mono("Error"),"callback_data":"s_err"}],
            [{"text":"⚠️ "+mono("Important Notice"),"callback_data":"s_warn"}],
            [{"text":"🎞 "+mono("Animation Text"),"callback_data":"anim_text"}],
...
def show_users_menu(chat_id, mid=None):
    ...
    default_txt = f"👥 **Users Overview:**\n\n• Total Active Users: `{cnt}`"
    txt, entities = _admin_template_payload("admin_users_text", default_txt, {"{count}": str(cnt)})
    data = {"chat_id": chat_id, "text": txt, "reply_markup": {"inline_keyboard": kb}}
    if mid: data["message_id"] = mid
    if entities: data["entities"] = entities
    else: data["parse_mode"] = "Markdown"
    ...
    if uid in ADMIN_IDS:
        ...
        if cdata == "adm_home":
            ...
            txt=(f"📊 **SYSTEM OVERVIEW**\n\n"
                 ...
                 f"📦 File expiry timer: `{file_timer}s`\n\n"
                 "🟢 Core services ready")
            kb=[[{"text":"🔄 Refresh","callback_data":"m_stats"}], [{"text":"⬅️ Back","callback_data":"adm_home"}]]
            tg_call("editMessageText",{"chat_id":cid,"message_id":mid,"text":txt,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(kb)}})
        ...
        elif cdata == "set_grp_cleanup":
            show_settings_group(cid,mid,"cleanup")
            ...
            ADMIN_SESSIONS[uid]={"step":"SET_VERIFY_RETRY"}; cur=_get_message_template("verify_retry_text", DEFAULT_VERIFY_RETRY)[0]; tg_call("sendMessage",{"chat_id":cid,"text":f"📝 CURRENT MESSAGE:\n\n{cur}\n\n✏️ Send the new Retry message:"})
        ...
        elif cdata == "user_cleanup_toggle":
            cur = get_setting("user_message_cleanup_enabled", "true").lower() == "true"
            set_setting("user_message_cleanup_enabled", "false" if cur else "true")
            show_settings_group(cid, mid, "cleanup")
        elif cdata == "user_cleanup_timer":
            kb=[[{"text":"30s","callback_data":"umt_30"},{"text":"1 min","callback_data":"umt_60"}],
                [{"text":"2 min (Default)","callback_data":"umt_120"},{"text":"3 min","callback_data":"umt_180"}],
                [{"text":"5 min","callback_data":"umt_300"},{"text":"10 min","callback_data":"umt_600"}],
                [{"text":"Custom","callback_data":"umt_custom"}],
                [{"text":"⬅️ "+mono("Back"),"callback_data":"set_grp_cleanup"}]]
            current = get_setting("user_message_cleanup_time", DEFAULT_USER_MSG_TIMER)
            tg_call("editMessageText", {"chat_id":cid,"message_id":mid,"text":f"⏱ **USER MESSAGE TIMER**\n\nCurrent: `{current}s`\n\nEach tracked bot message starts its own countdown when sent.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}})
        elif cdata.startswith("umt_"):
            value = cdata.split("_", 1)[1]
            if value == "custom":
                ADMIN_SESSIONS[uid] = {"step":"SET_USER_MSG_TIMER"}
                tg_call("sendMessage", {"chat_id":cid,"text":"Enter user-message cleanup time in seconds (1-86400):"})
            else:
                set_setting("user_message_cleanup_time", value)
                show_settings_group(cid, mid, "cleanup")
        elif cdata.startswith("toggle_cleanup_"):
            key = cdata.replace("toggle_", "")
            cur = get_setting(key, "true").lower() == "true"
            set_setting(key, "false" if cur else "true")
            show_settings_group(cid, mid, "cleanup")
        ...
        # --- Force Join Sub-Module ---
            ...
            cur = _get_message_template("force_join_text", DEFAULT_FJ)[0]
        ...
        # --- Broadcast Sub-Module ---
            ...
            failed = 0
            for u in users:
                target = u.get("telegram_user_id")
                if not target:
                    continue
                # A new broadcast starts a fresh chat state: remove every
                # tracked user/bot message first, including older broadcasts.
                _clear_chat_before_broadcast(target)
                result = tg_call("copyMessage", {
                    "chat_id": target,
                    "from_chat_id": bc_data["from_chat"],
                    "message_id": bc_data["mid"]
                })
                if result.get("ok"):
                    sent += 1
                    _remember_broadcast(target, result["result"].get("message_id"))
                else:
                    failed += 1
                time.sleep(0.05)
            tg_call("sendMessage", {"chat_id": cid, "text": f"📣 Broadcast complete\n\n✅ Sent: {sent}\n❌ Failed: {failed}\n🧹 Old chat messages cleared before each send."})
            show_admin_home(cid)
        ...
        # --- Settings Sub-Module ---
        elif cdata == "msg_studio":
            items = [
                ("👋 Welcome", "welcome_text", DEFAULT_WELCOME, "SET_W"),
                ("⚠️ Error", "error_feedback_text", DEFAULT_ERROR, "SET_E"),
                ("📢 Force Join Caption", "force_join_text", DEFAULT_FJ, "SET_FJ_CAPTION"),
                ("🔄 Retry", "verify_retry_text", DEFAULT_VERIFY_RETRY, "SET_VERIFY_RETRY"),
                ("⚠️ Important Notice", "auto_delete_warn_text", DEFAULT_WARN, "SET_WARN"),
                ("🎞 Animation Text", "anim_text", DEFAULT_ANIM_TEXT, "SET_ANIM_TEXT"),
            ]
            kb=[]
            for label, key, default, step in items:
                kb.append([{"text": label, "callback_data": f"editmsg_{key}"}])
            kb.append([{"text": "🔘 Verify Button Label", "callback_data": "verify_btn_edit"}])
            kb.append([{"text": "🛠 Admin Panel Text Studio", "callback_data": "admin_text_studio"}])
            kb.append([{"text": "⬅️ "+mono("Back"), "callback_data": "m_sett"}])
            tg_call("editMessageText", {"chat_id":cid,"message_id":mid,"text":"✨ **MESSAGE STUDIO**\n\nEdit every user-facing message.\n\n💎 **Custom/Premium Emoji:** send the replacement directly from Telegram. Its custom emoji entities + formatting will be saved and reused.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":mono_admin_kb(kb)}})
        elif cdata == "admin_text_studio":
            kb = []
            for label, key in _admin_editor_items():
                kb.append([{"text": label, "callback_data": f"editadmin_{key}"}])
            kb.append([{"text": "ℹ️ Use custom/premium emojis in the message you send. Entities are saved.", "callback_data": "noop"}])
            kb.append([{"text": "⬅️ "+mono("Back"), "callback_data": "msg_studio"}])
            tg_call("editMessageText", {"chat_id":cid,"message_id":mid,"text":"🛠 **ADMIN PANEL TEXT STUDIO**\n\nCustomize the main Admin Panel screens.\n\n💎 Send text containing Telegram Custom/Premium Emojis, bold, italic, links, etc. The original Telegram entities are stored and reused.","parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}})
        elif cdata.startswith("editadmin_"):
            key = cdata[len("editadmin_"):]
            defaults = {
                "admin_home_text": "✦ **LEXI CONTROL CENTER** ✦\n\n🟢 **SYSTEM ONLINE**\n\n👥 Users: `{users}`\n📦 Files: `{files}`\n📢 FJ Active: `{active}`\n🧹 Cleanup: `{cleanup}`\n⏱ Message TTL: `{msg_timer}s`\n📦 File TTL: `{file_timer}s`\n\nChoose a module below.",
                "admin_files_text": "✦ **FILE VAULT** ✦\n\nManage, publish, edit and retire your stored file links.",
                "admin_fj_text": "✦ **FORCE JOIN CONTROL** ✦\n\nConfigure channels, presentation and verification flow.",
                "admin_settings_text": "✦ **SETTINGS HUB** ✦\n\nEverything is grouped into clean modules.\nChanges apply immediately and are stored persistently.",
                "admin_users_text": "👥 **Users Overview:**\n\n• Total Active Users: `{count}`",
            ...
            if key in defaults:
                ADMIN_SESSIONS[uid] = {"step": "SET_ADMIN_TEXT", "key": key}
                cur = _get_message_template(key, defaults[key])[0]
                tg_call("sendMessage", {"chat_id":cid,"text":f"🛠 **EDIT {key.replace('admin_','').replace('_text','').upper()}**\n\nCurrent:\n{cur}\n\n💎 Send the replacement as a Telegram message.\nCustom/Premium Emojis + formatting + links will be preserved.\n\nNote: inline keyboard button labels cannot carry Telegram custom-emoji entities; screen/message text can.","parse_mode":"Markdown"})
        elif cdata.startswith("editmsg_"):
            key = cdata[len("editmsg_"):]
            meta = {
                "welcome_text": ("Welcome", DEFAULT_WELCOME, "SET_W"),
                "error_feedback_text": ("Error", DEFAULT_ERROR, "SET_E"),
                "force_join_text": ("Force Join Caption", DEFAULT_FJ, "SET_FJ_CAPTION"),
                "verify_retry_text": ("Retry", DEFAULT_VERIFY_RETRY, "SET_VERIFY_RETRY"),
                "auto_delete_warn_text": ("Important Notice", DEFAULT_WARN, "SET_WARN"),
                "anim_text": ("Animation Text", DEFAULT_ANIM_TEXT, "SET_ANIM_TEXT"),
            }.get(key)
            if meta:
                label, default, step = meta
                ADMIN_SESSIONS[uid] = {"step": step}
                cur = _get_message_template(key, default)[0]
                tg_call("sendMessage", {"chat_id":cid,"text":f"✨ **EDIT {label.upper()}**\n\nCurrent:\n{cur}\n\n💎 Send the new message now.\nCustom/Premium Emoji, formatting and links will be preserved.\n\nPlaceholders: `{{name}}` / `{{time}}` where supported."})
        elif cdata == "verify_btn_edit":
            ADMIN_SESSIONS[uid] = {"step": "SET_VERIFY_BTN"}
            cur = get_setting("verify_button_text", DEFAULT_VERIFY_BUTTON)
            tg_call("sendMessage", {"chat_id":cid,"text":f"🔘 **VERIFY BUTTON LABEL**\n\nCurrent: {cur}\n\n✏️ Send the new button label:"})
        elif cdata == "s_wel":
            ADMIN_SESSIONS[uid] = {"step": "SET_W"}
            cur = _get_message_template("welcome_text", DEFAULT_WELCOME)[0]
            ...
            cur = _get_message_template("error_feedback_text", DEFAULT_ERROR)[0]
            ...
            kb = [
                ...
                [{"text": "✏️ "+mono("Edit JOIN NOW Label"), "callback_data": "notice_btn_text_edit"}],
                [{"text": "🔗 "+mono("Edit JOIN NOW Link"), "callback_data": "notice_btn_edit"}, {"text": "🗑 "+mono("REMOVE JOIN NOW"), "callback_data": "notice_btn_remove"}],
                [{"text": "⬅️ "+mono("Back"), "callback_data": "m_sett"}]
            ]
            ...
            cur = _get_message_template("auto_delete_warn_text", DEFAULT_WARN)[0]
            ...
            show_settings_menu(cid)
        elif cdata == "notice_btn_text_edit":
            ADMIN_SESSIONS[uid] = {"step": "SET_NOTICE_BTN_TEXT"}
            cur = get_setting("notice_btn_text", DEFAULT_NOTICE_BTN_TEXT)
            tg_call("sendMessage", {"chat_id":cid,"text":f"🔘 **JOIN NOW BUTTON LABEL**\n\nCurrent: {cur}\n\n✏️ Send the new label (or `none` to hide):"})
...
def process_message_update(data):
    ...
    # Native Admin Panel reply-keyboard navigation. Button presses are plain
    # text messages; resolve them against the current screen's route map.
    # Never consume a message while an edit/input session is active.
    if uid in ADMIN_IDS and not ADMIN_SESSIONS:
        route = _admin_route_text(cid, txt)
        if route:
            cq = {
                "id": f"reply:{uid}:{mid}",
                "from": {"id": uid},
            ...
            _safe_run(process_callback_query, cq)
            # Reply-keyboard button presses are ordinary admin messages. Remove
            # the navigation tap so the chat contains only the current menu +
            # the useful details/prompts.
            try:
        ...
        # Main Admin Panel shortcuts are always available even before the
        # first inline-derived screen has established a route map.
        shortcut_routes = {
            "📦 Files": "m_files", "🛠 Admin Panel": "adm_home",
            "📢 Force Join": "m_fj", "📣 Broadcast": "m_bc",
            "👥 Users": "m_users", "⚙️ Settings": "m_sett",
            "📊 Analytics": "m_stats", "🧹 Cleanup": "set_grp_cleanup",
            "✨ Message Studio": "msg_studio", "🛠 System": "set_system",
        }
        route = shortcut_routes.get(txt.strip())
        if route:
            cq = {"id": f"shortcut:{uid}:{mid}", "from": {"id": uid},
                  "message": {"chat": {"id": cid}, "message_id": mid}, "data": route}
            _safe_run(process_callback_query, cq)
            return "OK", 200

    if uid in ADMIN_IDS and txt.strip() == "⌨️ Hide Admin Panel":
        ADMIN_SESSIONS.pop(uid, None)
        with ADMIN_REPLY_LOCK:
            ADMIN_REPLY_ROUTES.pop(str(cid), None)
            ADMIN_REPLY_KEYBOARDS.pop(str(cid), None)
        _hide_admin_reply_keyboard(cid)
        return "OK", 200
    ...
    if uid in ADMIN_IDS and uid in ADMIN_SESSIONS:
        ...
        if st == "SET_VERIFY_RETRY":
            _set_message_template("verify_retry_text", msg); ADMIN_SESSIONS.pop(uid,None); tg_call("sendMessage",{"chat_id":cid,"text":"✅ Retry message updated."}); show_settings_menu(cid); return "OK",200
        elif st == "F_TITLE":
        ...
        # Force Join Caption & Poster
            ...
            _set_message_template("force_join_text", msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id": cid, "text": "Success"})
            show_fj_menu(cid)
        ...
        # Settings Inputs
        elif st == "SET_ADMIN_TEXT":
            key = sess.get("key")
            if key:
                _set_message_template(key, msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id":cid,"text":"✅ Admin Panel text updated. Custom/Premium Emoji entities are saved."})
            show_admin_home(cid)

        elif st == "SET_W":
            _set_message_template("welcome_text", msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id": cid, "text": "Success"})
            show_settings_menu(cid)
        elif st == "SET_E":
            _set_message_template("error_feedback_text", msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id": cid, "text": "Success"})
            show_settings_menu(cid)
        elif st == "SET_WARN":
            _set_message_template("auto_delete_warn_text", msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id": cid, "text": "Success!"})
            show_settings_menu(cid)
        ...
        elif st == "SET_USER_MSG_TIMER":
            try:
                s_val = int(txt.strip())
                if s_val < 1 or s_val > 86400:
                    raise ValueError
                set_setting("user_message_cleanup_time", str(s_val))
                ADMIN_SESSIONS.pop(uid, None)
                tg_call("sendMessage", {"chat_id": cid, "text": "✅ User-message cleanup timer updated."})
                show_settings_group(cid, mid, "cleanup")
            except Exception:
                tg_call("sendMessage", {"chat_id": cid, "text": "⚠️ Enter a whole number from 1 to 86400 seconds:"})
            ...
            tg_call("sendMessage", {"chat_id": cid, "text": "Success"})
            show_settings_menu(cid)
        elif st == "SET_VERIFY_BTN":
            value = txt.strip() or DEFAULT_VERIFY_BUTTON
            set_setting("verify_button_text", value)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id":cid,"text":"✅ Verify button label updated."})
            show_settings_group(cid, mid, "msg")
        elif st == "SET_NOTICE_BTN_TEXT":
            value = txt.strip()
            set_setting("notice_btn_text", "none" if value.lower() == "none" else value)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id":cid,"text":"✅ JOIN NOW button label updated."})
            show_settings_group(cid, mid, "msg")
            ...
            if not re.match(r"^https?://\S+$", url):
                ...
                if not get_setting("notice_btn_text", "").strip():
            ...
            _set_message_template("anim_text", msg)
            ADMIN_SESSIONS.pop(uid, None)
            tg_call("sendMessage", {"chat_id": cid, "text": "Success"})
            show_settings_menu(cid)
    ...
    if txt.startswith("/start"):
        ...
        try:
            ...
            if code:
                ...
                welcome, welcome_entities = _template_payload("welcome_text", DEFAULT_WELCOME)
                welcome_payload = {"chat_id": cid, "text": welcome}
                if welcome_entities:
                    welcome_payload["entities"] = welcome_entities
                else:
                    welcome_payload["parse_mode"] = "Markdown"
                result = tg_call("sendMessage", welcome_payload)
                if result.get("ok") and get_setting("cleanup_flow", "true").lower() == "true":
                    _schedule_public_message_delete(cid, result["result"].get("message_id"))
            ...
            traceback.print_exc()
            error_text, error_entities = _template_payload("error_feedback_text", DEFAULT_ERROR)
            error_payload = {"chat_id": cid, "text": error_text}
            if error_entities:
                error_payload["entities"] = error_entities
            else:
                error_payload["parse_mode"] = "Markdown"
            result = tg_call("sendMessage", error_payload)
            if result.get("ok") and get_setting("cleanup_flow", "true").lower() == "true":
                _schedule_public_message_delete(cid, result["result"].get("message_id"))
            if not result.get("ok"):
