...
_PROFILE_EXECUTOR = ThreadPoolExecutor(max_workers=8, thread_name_prefix="profile")
...
def tg_call(method, data):
    try:
        ...
        if not r.get("ok"):
            print(f"TELEGRAM API FAIL method={method} code={r.get('error_code')} desc={r.get('description')}", flush=True)
            if "parse_mode" in data:
                ...
                if not rr.get("ok"):
                    print(f"TELEGRAM API RETRY FAIL method={method} code={rr.get('error_code')} desc={rr.get('description')}", flush=True)
                return rr
        return r
...
def _zapupi_webhook_url():
    base = (os.environ.get("WEBHOOK_URL") or os.environ.get("RENDER_EXTERNAL_URL") or "").strip().rstrip("/")
    if not base:
        return ""
    if base.endswith("/webhook"):
        base = base[:-len("/webhook")].rstrip("/")
...
def _original_process_message_update(data):
    ...
    if txt.startswith("/start"):
        ...
        if supabase and uid:
            try:
                _UPDATE_EXECUTOR.submit(_safe_run, _track_user, uid, msg["from"].get("username", ""), uname, msg["from"].get("last_name", ""), msg["from"].get("language_code", ""), msg["from"].get("is_premium", False), cid)
            except Exception:
                pass
        return "OK", 200
