...
def _colorize_button(btn):
    ...
    if cb:
        ...
        # Idempotent: already-rendered buttons must never accumulate markers.
        if not any(text_value.startswith(mark + " ") or text_value.startswith(mark)
                   for mark in BUTTON_COLOR_CHOICES.values()):
            marker = BUTTON_COLOR_CHOICES.get(_button_color(cb, text_value), "🔵")
            b["text"] = marker + " " + text_value
    return b
...
def _fj_keyboard(unjoined, code):
    ...
    for c in unjoined:
        ...
        row.append({"text": BUTTON_COLOR_CHOICES.get(_button_color(f"fj_slot_{c.get('slot')}", btn_txt), "🔵") + " " + btn_txt, "url": c.get("join_url")})
        if len(row) == layout:
            kb.append(row); row = []
...
def _broadcast_history_save(history):
    try:
        set_setting(BROADCAST_HISTORY_KEY, json.dumps(history, separators=(",", ":")))
        return True
    except Exception as exc:
...
def _delete_previous_broadcasts(history):
    """Delete every message recorded for the previous broadcast before sending a new one."""
    if not history:
        return True, 0
    jobs = [(int(chat_id), int(mid)) for chat_id, mids in history.items() for mid in mids]
    failed = []
    lock = threading.Lock()

    def remove_one(item):
        chat_id, message_id = item
        r = tg_call("deleteMessage", {"chat_id": chat_id, "message_id": message_id})
        if not r.get("ok"):
            # A message that Telegram says is already gone is effectively deleted.
            desc = str(r.get("description", "")).lower()
            if "message to delete not found" not in desc and "message can't be deleted" not in desc:
                with lock:
                    failed.append(item)

    workers = min(8, max(1, len(jobs)))
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="broadcast-delete") as ex:
        list(ex.map(remove_one, jobs))
    if failed:
        print(f"BROADCAST OLD MESSAGE DELETE FAILED count={len(failed)}", flush=True)
...
def process_callback_query(cq):
    ...
    if uid in ADMIN_IDS:
        ...
        # --- Broadcast Sub-Module ---
            ...
            # HARD ORDER: delete every message from the previous broadcast first.
            # If even one recorded deletion fails, do NOT send the new broadcast.
            previous = _broadcast_history_load()
            ok, deleted_count = _delete_previous_broadcasts(previous)
            if not ok:
                tg_call("sendMessage", {
                    "chat_id": cid,
                    "text": f"❌ Old broadcast cleanup failed ({deleted_count} message(s)).\nNew broadcast was NOT sent."
                })
                show_admin_home(cid)
                return "OK", 200
            _broadcast_history_save({})
                ...
                try:
                    ...
                    if len(message_ids) > 1:
                        ...
                        if r.get("ok"):
                            ...
                            # Compatibility fallback: preserve every album item if copyMessages is unavailable.
                            copied = []
            ...
            # Persist the exact newly-created message IDs so the next broadcast
            # can remove these messages before delivering its replacement.
            _broadcast_history_save(new_history)
            ADMIN_SESSIONS.pop(uid, None)
            pmid = ADMIN_PROMPT_MESSAGES.pop(uid, None)
            if pmid:
                tg_call("deleteMessage", {"chat_id": cid, "message_id": pmid})
