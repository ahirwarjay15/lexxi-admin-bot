...
BROADCAST_DRAFTS_KEY = "broadcast_drafts_v1"
TELEGRAM_BLOCKED_USERS_KEY = "telegram_blocked_users_v1"
...
def _broadcast_history_load():
    """Load persistent multi-broadcast history.

    v3 format:
      [{"id": "...", "created_at": "...", "messages": {"user_id": [message_ids]}}]
    The loader also migrates the old v2 single-broadcast dict.
    """
    try:
                ...
                for item in data:
                    ...
                    for chat_id, mids in messages.items():
                        try:
                            ...
                            if vals:
                                clean[str(chat_id)] = vals
                        except Exception:
                            continue
                    if clean:
                        out.append({
                            "id": str(item.get("id") or f"{item.get('created_at','')}-{len(out)}"),
                            "created_at": str(item.get("created_at") or ""),
                            "messages": clean,
                        })
                return out
            # Graceful migration from the old v2 dict.
        ...
        # Also check the previous key if this installation was upgraded in place.
        legacy_raw = get_setting("last_broadcast_messages_v2", "")
        if legacy_raw:
...
def _broadcast_history_save(history):
    try:
        return bool(set_setting(
            BROADCAST_HISTORY_KEY,
            json.dumps(history if isinstance(history, list) else [], separators=(",", ":"))
        ))
    except Exception as exc:
...
def _broadcast_drafts_load():
    try:
        data = _load_json_setting(BROADCAST_DRAFTS_KEY, {})
        return data if isinstance(data, dict) else {}
    except Exception:
        return {}


def _broadcast_draft_get(uid):
    return _broadcast_drafts_load().get(str(uid))


def _broadcast_draft_save(uid, payload):
    try:
        drafts = _broadcast_drafts_load()
        drafts[str(uid)] = payload
        return bool(set_setting(BROADCAST_DRAFTS_KEY, json.dumps(drafts, separators=(",", ":"))))
    except Exception as exc:
        print(f"BROADCAST DRAFT SAVE ERROR user={uid}: {exc}", flush=True)
        return False


def _broadcast_draft_clear(uid):
    try:
        drafts = _broadcast_drafts_load()
        drafts.pop(str(uid), None)
        return bool(set_setting(BROADCAST_DRAFTS_KEY, json.dumps(drafts, separators=(",", ":"))))
    except Exception as exc:
        print(f"BROADCAST DRAFT CLEAR ERROR user={uid}: {exc}", flush=True)
        return False


def _delete_broadcast_record(record):
    if not isinstance(record, dict):
        return True, 0
    return _delete_previous_broadcasts(record.get("messages") or {})


def _delete_broadcast_history(history, mode):
    """Delete exactly the selected previous broadcast scope."""
    ...
    if mode == "last":
        target = history[-1]
        ok, deleted = _delete_broadcast_record(target)
        return ok, deleted, (history[:-1] if ok else history)

    if mode == "all":
        total = 0
        remaining = []
        for idx, record in enumerate(history):
            ok, deleted = _delete_broadcast_record(record)
            total += deleted
            if not ok:
                # Preserve only records we did not successfully finish.
                remaining.extend(history[idx:])
                return False, total, remaining
        return True, total, []

    return True, 0, history


def _delete_previous_broadcasts(history):
    """Delete a chat->message_ids map, before a new broadcast is delivered."""
    if not history:
        return True, 0
    total = 0
    failed = []
        ...
        for i in range(0, len(ids), 100):
            ...
            for message_id in batch:
                ...
                if any(x in desc for x in (
                    "message to delete not found", "message_id_invalid",
                    "message id invalid", "message can't be deleted"
                )):
                    total += 1
                else:
...
def _telegram_blocked_load():
    value = _load_json_setting(TELEGRAM_BLOCKED_USERS_KEY, [])
...
def _telegram_blocked_save(users):
    return set_setting(
        TELEGRAM_BLOCKED_USERS_KEY,
        json.dumps(sorted({int(x) for x in users}), separators=(",", ":"))
    )


def _broadcast_confirm(cid, uid):
    sess = ADMIN_SESSIONS.get(uid, {})
    payload = sess.get("bc_payload") or _broadcast_draft_get(uid) or {}
    ...
    if not mids:
        ...
        _broadcast_draft_clear(uid)
        _delete_admin_menu(uid, cid)
        show_admin_home(cid)
        return

    payload["message_ids"] = [int(x) for x in mids]
    payload["from_chat"] = int(payload.get("from_chat", cid))
    sess["bc_payload"] = payload
    _broadcast_draft_save(uid, payload)
    ...
    kb = [
        ...
        [{"text": "🗑 Delete LAST 1 Broadcast", "callback_data": "bc_cleanup_last"}],
        [{"text": "❌ Cancel", "callback_data": "adm_cancel"}],
    ]
    r = tg_call("sendMessage", {
        "chat_id": cid,
        "text": (
            f"📣 Broadcast ready ({len(mids)} message{'s' if len(mids) != 1 else ''}).\n\n"
            "Choose what to clear before sending:"
        ),
...
def _broadcast_final_confirm(cid, uid, mode):
    sess = ADMIN_SESSIONS.get(uid, {})
    payload = sess.get("bc_payload") or _broadcast_draft_get(uid) or {}
    mids = payload.get("message_ids") or []
    if not mids:
        tg_call("sendMessage", {"chat_id": cid, "text": "❌ Broadcast draft not found. Please start again."})
        return
    payload["cleanup_mode"] = mode
    sess["bc_payload"] = payload
    _broadcast_draft_save(uid, payload)
    _delete_admin_menu(uid, cid)
    label = "ALL previous broadcasts" if mode == "all" else "LAST 1 previous broadcast"
    kb = [
        [{"text": "✅ Confirm & Send", "callback_data": "send_confirmed_bc"}],
        [{"text": "⬅️ Change Cleanup", "callback_data": "bc_choose_cleanup"}],
        [{"text": "❌ Cancel", "callback_data": "adm_cancel"}],
    ]
    r = tg_call("sendMessage", {
        "chat_id": cid,
        "text": (
            f"📣 FINAL BROADCAST CONFIRMATION\n\n"
            f"🧹 Cleanup: {label}\n"
            f"📦 Content: {len(mids)} source message{'s' if len(mids) != 1 else ''}\n\n"
            "Cleanup will finish first. If cleanup fails, the new broadcast will NOT be sent."
        ),
        "reply_markup": {"inline_keyboard": mono_admin_kb(kb)}
    })
    if r.get("ok"):
...
def _finish_broadcast_album(uid, cid, group_id):
    ...
    if not sess or sess.get("step") != "WAIT_BC_CONTENT":
        return
    payload = {"from_chat": cid, "message_ids": message_ids, "media_group_id": group_id}
    sess["bc_payload"] = payload
    _broadcast_draft_save(uid, payload)
...
def show_users_menu(chat_id, mid=None):
    ...
    if supabase:
        try:
            ...
            total = 0
    bot_blocked = len(_blocked_users_load())
    telegram_blocked = len(_telegram_blocked_load())
    txt = (
        "👥 **Users Overview:**\n\n"
        f"• Total Users: `{total}`\n"
        f"• 🚫 Bot Blocked: `{bot_blocked}`\n"
        f"• 📵 Telegram Blocked (tracked): `{telegram_blocked}`\n\n"
        "Telegram-blocked users are detected from broadcast delivery failures."
    )
    kb = [
        [{"text": "📜 View All Users", "callback_data": "u_list_0"}],
        [{"text": "🚫 Bot Blocked List", "callback_data": "u_blocked_list_0"}],
        [{"text": "📵 Telegram Blocked List", "callback_data": "u_tgblocked_list_0"}],
        [{"text": "⬅️ "+mono("Back"), "callback_data": "adm_home"}]
    ]
    data={"chat_id":chat_id,"message_id":mid,"text":txt,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}} if mid else {"chat_id":chat_id,"text":txt,"parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}}
    tg_call("editMessageText" if mid else "sendMessage", data)
...
def process_callback_query(cq):
    ...
    if uid in ADMIN_IDS:
        ...
        if cdata == "adm_cancel":
            ...
            sess=ADMIN_SESSIONS.pop(uid,{})
            if sess.get("bc_payload") or _broadcast_draft_get(uid):
                _broadcast_draft_clear(uid)
            pmid=ADMIN_PROMPT_MESSAGES.pop(uid,None)
        ...
        if cdata == "adm_home":
            ...
            if oldp:
                tg_call("deleteMessage", {"chat_id": cid, "message_id": oldp})
            # Starting a new broadcast intentionally replaces any stale draft for this admin.
            _broadcast_draft_clear(uid)
        ...
        # --- Broadcast Sub-Module ---
        elif cdata == "bc_choose_cleanup":
            _broadcast_confirm(cid, uid)

        elif cdata in ("bc_cleanup_all", "bc_cleanup_last"):
            mode = "all" if cdata == "bc_cleanup_all" else "last"
            if not (ADMIN_SESSIONS.get(uid, {}).get("bc_payload") or _broadcast_draft_get(uid)):
                tg_call("sendMessage", {"chat_id": cid, "text": "❌ Broadcast draft not found. Please start again."})
                show_admin_home(cid)
                return "OK", 200
            _broadcast_final_confirm(cid, uid, mode)

        elif cdata == "send_confirmed_bc":
            sess = ADMIN_SESSIONS.setdefault(uid, {"return_callback": "adm_home", "chat_id": cid})
            bc_data = sess.get("bc_payload") or _broadcast_draft_get(uid)
            if not bc_data:
                tg_call("sendMessage", {"chat_id": cid, "text": "❌ Broadcast draft not found. Please start again."})
                show_admin_home(cid)
                return "OK", 200

            cleanup_mode = bc_data.get("cleanup_mode")
            if cleanup_mode not in ("all", "last"):
                _broadcast_confirm(cid, uid)
                return "OK", 200
            ...
            try:
                history = _broadcast_history_load()
                ok, deleted_count, remaining_history = _delete_broadcast_history(history, cleanup_mode)
                if not ok:
                    tg_call("sendMessage", {
                        "chat_id": cid,
                        "text": f"❌ Selected old-broadcast cleanup failed ({deleted_count} messages deleted before failure).\nNew broadcast was NOT sent."
                    })
                    return "OK", 200

                # Save the post-cleanup history before sending. If this write fails,
                # stop rather than sending a broadcast whose cleanup state is unknown.
                if not _broadcast_history_save(remaining_history):
                    tg_call("sendMessage", {"chat_id": cid, "text": "❌ Broadcast history storage failed. New broadcast was NOT sent."})
                    return "OK", 200
                ...
                telegram_blocked = _telegram_blocked_load()
                users = supabase.table("users").select("telegram_user_id").execute().data or [] if supabase else []
                clean_users = []
                for u in users:
                    try:
                        target = int(u.get("telegram_user_id"))
                    except Exception:
                        continue
                    if target not in blocked:
                        clean_users.append({"telegram_user_id": target})
                ...
                def deliver_to_user(u):
                    ...
                    try:
                        if len(message_ids) > 1:
                            ...
                            if r.get("ok"):
                                copied = [
                                    int(x.get("message_id"))
                                    for x in (r.get("result") or [])
                                    if isinstance(x, dict) and x.get("message_id")
                                ]
                            else:
                                for source_mid in message_ids:
                                    rr = tg_call("copyMessage", {
                                        "chat_id": target,
                                        "from_chat_id": bc_data["from_chat"],
                                        "message_id": source_mid
                                    })
                                    if not rr.get("ok"):
                                        error_desc = str(rr.get("description", "copy failed"))
                                        break
                                    copied.append(int(rr["result"]["message_id"]))
                        else:
                            r = tg_call("copyMessage", {
                                "chat_id": target,
                                "from_chat_id": bc_data["from_chat"],
                                "message_id": message_ids[0]
                            })
                            if not r.get("ok"):
                                error_desc = str(r.get("description", "copy failed"))
                            else:
                                copied = [int(r["result"]["message_id"])]

                        if copied and not error_desc:
                            with result_lock:
                                new_history_map[str(target)] = copied
                                sent_count += 1
                            return

                        # Never leave a partial album/message set behind for a failed user.
                        if copied:
                            _delete_previous_broadcasts({str(target): copied})
                        desc_low = error_desc.lower()
                        is_blocked_error = any(x in desc_low for x in (
                        ...
                        with result_lock:
                            failed_count += 1
                            failed_now.add(target)
                            if is_blocked_error:
                                blocked_count += 1
                                blocked_now.add(target)
                    except Exception as exc:
                        with result_lock:
                            failed_count += 1
                            failed_now.add(target)
                        print(f"BROADCAST DELIVERY ERROR user={target}: {exc}", flush=True)

                workers = min(8, max(1, len(clean_users)))
                if clean_users:
                    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="broadcast-send") as ex:
                        list(ex.map(deliver_to_user, clean_users))

                # Successful delivery means Telegram did not reject the user, so remove
                # stale "blocked" tracking if they later became reachable again.
                telegram_blocked.difference_update({int(x) for x in new_history_map.keys()})
                telegram_blocked.update(blocked_now)
                if not _telegram_blocked_save(telegram_blocked):
                    # Tracking failure is non-fatal to delivery, but is reported.
                    print("TELEGRAM BLOCKED USER TRACKING SAVE FAILED", flush=True)

                new_record = {
                    "id": f"bc-{int(time.time()*1000)}",
                    "created_at": str(int(time.time())),
                    "messages": new_history_map,
                }
                final_history = list(remaining_history) + ([new_record] if new_history_map else [])

                if not _broadcast_history_save(final_history):
                    rollback_ok, _ = _delete_previous_broadcasts(new_history_map)
                    tg_call("sendMessage", {
                        "chat_id":cid,
                        "text": (
                            "❌ Broadcast history could not be saved; the new broadcast was rolled back."
                            if rollback_ok else
                            "❌ Broadcast history could not be saved and rollback was incomplete. Do not send another broadcast until storage is fixed."
                        )
                    })
                    return "OK",200
                ...
                tg_call("sendMessage", {
                    ...
                    "text": (
                        ...
                        f"🚫 Telegram Blocked: {blocked_count}\n"
                        f"🧹 Cleanup: {'ALL' if cleanup_mode == 'all' else 'LAST 1'}"
                    )
                })
                show_admin_home(cid)
            finally:
        ...
        # --- Users Sub-Module ---
            ...
            _show_users_page(cid, mid, 0, mode="all")
        elif cdata.startswith("u_list_"):
            try:
                page=int(cdata.rsplit("_",1)[1])
            except Exception:
                page=0
            _show_users_page(cid, mid, max(0,page), mode="all")
        elif cdata.startswith("u_blocked_list_"):
            try:
                page=int(cdata.rsplit("_",1)[1])
            except Exception:
                page=0
            _show_users_page(cid, mid, max(0,page), mode="bot_blocked")
        elif cdata.startswith("u_tgblocked_list_"):
            try:
                page=int(cdata.rsplit("_",1)[1])
            except Exception:
                page=0
            _show_users_page(cid, mid, max(0,page), mode="telegram_blocked")
...
def _track_user(uid, username="", first_name="User", last_name=""):
    """Create/update a user record without requiring a schema migration."""
    if not supabase:
        return False
    uid = int(uid)
    payload = {
    ...
    try:
        try:
            ...
            # Older schemas may only have telegram_user_id/username/first_name.
            payload.pop("last_name", None)
            supabase.table("users").upsert(payload, on_conflict="telegram_user_id").execute()
        return True
    except Exception as exc:
        print(f"TRACK USER ERROR user={uid}: {exc}", flush=True)
        return False


def _show_users_page(cid, mid, page=0, page_size=20, mode="all"):
    page = max(0, int(page))
    if mode == "bot_blocked":
        ids = sorted(_blocked_users_load())
        total = len(ids)
        ids = ids[page*page_size:(page+1)*page_size]
        rows = []
        if ids and supabase:
            try:
                rows = supabase.table("users").select("*").in_("telegram_user_id", ids).execute().data or []
            except Exception:
                rows = [{"telegram_user_id": x} for x in ids]
        by_id = {int(r.get("telegram_user_id")): r for r in rows if r.get("telegram_user_id") is not None}
        rows = [by_id.get(x, {"telegram_user_id": x}) for x in ids]
    elif mode == "telegram_blocked":
        ids = sorted(_telegram_blocked_load())
        total = len(ids)
        ids = ids[page*page_size:(page+1)*page_size]
        rows = []
        if ids and supabase:
        ...
        rows, total = _load_users(page, page_size)

    if not rows:
        title = {
            "all": "👥 USERS",
            "bot_blocked": "🚫 BOT BLOCKED USERS",
            "telegram_blocked": "📵 TELEGRAM BLOCKED USERS"
        }.get(mode, "👥 USERS")
        tg_call("editMessageText", {
            "chat_id": cid, "message_id": mid, "text": f"{title}\n\nNo users found.",
            "reply_markup": {"inline_keyboard": [[{"text":"⬅️ Back","callback_data":"m_users"}]]}
        })
        return
    ...
    for u in rows:
        ...
        prefix = "🚫 " if target in _blocked_users_load() else ("📵 " if target in _telegram_blocked_load() else "👤 ")
        kb.append([{"text": f"{prefix}{name}"[:64], "callback_data": f"u_view_{target}"}])

    pages = max(1, (total + page_size - 1) // page_size)
    nav = []
    if page > 0:
        nav.append({"text":"◀️ Previous","callback_data":f"u_{mode}_list_{page-1}"})
    if (page + 1) * page_size < total:
        nav.append({"text":"Next ▶️","callback_data":f"u_{mode}_list_{page+1}"})
    if nav:
        kb.append(nav)
...
def _show_user_detail(cid, mid, target):
    ...
    if supabase:
        try:
            rows = supabase.table("users").select("*").eq("telegram_user_id", target).limit(1).execute().data or []
            if rows:
                row = rows[0]
        except Exception:
            pass
    name = clean_markdown(_user_display_name(row) if row else f"User {target}")
    username = clean_markdown(row.get("username")) if row.get("username") else "—"
    bot_blocked = target in _blocked_users_load()
    tg_blocked = target in _telegram_blocked_load()
    txt = (
        "👤 **USER DETAILS**\n\n"
        f"Name: {name}\n"
        f"Username: @{username}\n"
        f"Telegram ID: `{target}`\n"
        f"🚫 Bot Blocked: `{ 'YES' if bot_blocked else 'NO' }`\n"
        f"📵 Telegram Blocked (tracked): `{ 'YES' if tg_blocked else 'NO' }`"
    )
    kb = [
        [{"text":"🔓 Unblock User" if bot_blocked else "🚫 Block User",
          "callback_data":f"u_unblock_{target}" if bot_blocked else f"u_block_{target}"}],
        [{"text":"⬅️ Back","callback_data":"u_list_0"}]
    ]
    tg_call("editMessageText", {
        "chat_id":cid, "message_id":mid, "text":txt,
...
def process_message_update(data):
    ...
    if uid in ADMIN_IDS and uid in ADMIN_SESSIONS:
        ...
        # Broadcast Execution
            ...
            payload = {"from_chat": cid, "message_ids": [mid]}
            sess["bc_payload"] = payload
            _broadcast_draft_save(uid, payload)
            _broadcast_confirm(cid, uid)
