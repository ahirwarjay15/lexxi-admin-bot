...
def run_verify_process(chat_id, user_id, code, fj_mid):
    ...
    anim_base = get_setting("anim_text", DEFAULT_ANIM_TEXT).strip() or "Verifying"
    frames = [f"{anim_base}.", f"{anim_base}..", f"{anim_base}..."]
    ...
    def animate_loop():
        nonlocal anim_mid
        res = tg_call("sendMessage", {"chat_id": chat_id, "text": frames[0]})
        if not res.get("ok"):
            return
        anim_mid = res["result"]["message_id"]
        idx = 1
        while not stop_event.wait(anim_speed):
    ...
    if anim_enabled:
        anim_thread = threading.Thread(target=animate_loop, daemon=True)
        anim_thread.start()
    ...
    if anim_enabled:
        # Always allow at least one full 3-frame loop to be visible.
        minimum_animation_time = max(0.30, anim_speed * 3)
        elapsed = time.time() - verification_started
        if elapsed < minimum_animation_time:
            time.sleep(minimum_animation_time - elapsed)

        stop_event.set()
        if anim_thread:
            anim_thread.join(timeout=max(0.50, anim_speed + 0.20))
        if anim_mid:
