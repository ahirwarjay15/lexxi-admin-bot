...
_UPDATE_EXECUTOR = ThreadPoolExecutor(max_workers=8)
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if cdata.startswith("v:"):
            ...
            _UPDATE_EXECUTOR.submit(_safe_run, run_verify_process, cid, uid, code, mid)
        else:
            _UPDATE_EXECUTOR.submit(_safe_run, process_callback_query, cq)
        return "OK", 200

    # Message Router: use a persistent worker pool instead of daemon threads.
    # Daemon request threads can be lost during Gunicorn worker lifecycle events.
    if "message" in data:
