...
def _button_color(callback, label=None, default=None):
    ...
    if key not in BUTTON_COLORS:
        ...
        if saved in BUTTON_COLOR_CHOICES:
            ...
            elif any(x in low for x in ("cancel","back","remove","delete","off")) or "adm_cancel" in cb:
...
def _colorize_button(btn):
    ...
    if cb:
        marker = BUTTON_COLOR_CHOICES.get(_button_color(cb, b.get("text")), "🔵")
        b["text"] = marker + " " + str(b.get("text", ""))
    return b
