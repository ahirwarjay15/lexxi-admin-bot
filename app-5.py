...
# Deletions/old-menu cleanup are never allowed to sit in front of a user-visible send.
...
def _admin_clear_menu(chat_id):
    # Never put deletion in front of a new menu send. Remove the old menu in
    # the cleanup pool so navigation remains responsive.
...
def _clear_key_flow(chat_id):
    ...
    if not mids:
        return
    # Cleanup is deliberately fire-and-forget: the next menu/message is sent
    # first, so a delete call can never create perceived tap latency.
    for mid in mids:
        try:
            _async_delete_message(chat_id, mid)
        except Exception:
...
# User-facing language is kept in memory first (zero DB wait) and persisted
# asynchronously. Only English and Hindi are supported.
USER_LANGUAGE_CACHE = {}
...
def _set_user_lang(user_id, lang):
    ...
    with USER_LANGUAGE_LOCK:
        USER_LANGUAGE_CACHE[uid]=lang
    # Never make a tap wait on Supabase.
    try:
...
def _key_text(name, default=None, lang="en", **kwargs):
    lang=lang if lang in ("en","hi") else "en"
    # Admin custom English messages remain compatible. Hindi can override with
    # keymsg_hi_<name>; otherwise built-in Hindi is used for every user-facing key menu.
...
def _duration_details(chat_id,pid,did,lang="en"):
    ...
    # The first button is intentionally informational and has no callback.
