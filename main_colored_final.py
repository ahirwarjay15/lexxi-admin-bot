...
# --- Per-button Telegram styles ---
# Telegram Bot API 9.6 supports primary (blue), success (green), danger (red),
# and default button styles. Settings are persisted in the existing settings table.
BUTTON_STYLE_CHOICES = ("default", "primary", "success", "danger")
BUTTON_COLOR_LABELS = {
    "adm_home":"Admin Home", "m_files":"Files", "m_fj":"Force Join", "m_bc":"Broadcast",
    "m_users":"Users", "m_sett":"Settings", "m_stats":"Statistics",
    "f_add":"Add File", "f_del":"Remove File", "f_edit":"Edit File", "f_list":"List Files",
    "fj_caption":"Caption", "fj_poster":"Poster", "fj_add":"Add Channel Slot", "fj_edit":"Edit Channel Slot",
    "fj_onoff":"Channel ON/OFF", "fj_rem":"Remove Channel", "fjp_add":"Set/Replace Poster", "fjp_rem":"Remove Poster",
    "fmode_copy":"Copy File Mode", "fmode_forward":"Forward File Mode", "msg_retry":"Retry Message",
    "notice_btn_edit":"Edit JOIN NOW", "notice_btn_on":"JOIN NOW ON", "notice_btn_off":"JOIN NOW OFF", "notice_btn_remove":"Remove JOIN NOW",
    "notice_on":"Notice ON", "notice_off":"Notice OFF", "notice_text":"Edit Notice Text",
    "s_anim":"Animation", "s_err":"Error Message", "s_layout":"Force Join Layout", "s_timer":"Delete Timer",
    "s_upbtn":"Update Button", "s_warn":"Important Notice", "s_wel":"Welcome Message", "send_confirmed_bc":"Confirm Broadcast",
    "set_anim":"Animation Settings", "set_cleanup":"Message Cleanup", "set_system":"System Info", "set_verify":"Verification Settings",
    "setlay_1":"1 Button Per Row", "setlay_2":"2 Buttons Per Row", "st_30":"30 Seconds", "st_60":"1 Minute",
    "st_120":"2 Minutes", "st_180":"3 Minutes", "st_300":"5 Minutes", "st_custom":"Custom Timer", "u_list":"User List",
    "verify_method":"Verification Method", "verify_off":"Verification OFF", "verify_on":"Verification ON",
    "vm_both":"Both Verification", "vm_join":"Joined Only", "vm_req":"Request Only", "anim_off":"Animation OFF",
    "anim_on":"Animation ON", "anim_preview":"Animation Preview", "anim_speed":"Animation Speed", "anim_text":"Animation Text",
    "aspeed_0.10":"0.10s Speed", "aspeed_0.25":"0.25s Speed", "aspeed_0.50":"0.50s Speed", "aspeed_1.00":"1.00s Speed", "aspeed_custom":"Custom Speed",
    "fj_list_to_off":"List Inactive Channels", "fj_list_to_on":"List Active Channels"
}
BUTTON_COLOR_PREFIX_LABELS = {
    "v:":"Verify (*code)", "cf_del_":"Select Channel Remove", "cf_fdel_":"Select File Remove",
    "cf_off_":"Select Channel OFF", "cf_on_":"Select Channel ON", "act_del_":"Confirm Channel Delete",
    "act_fdel_":"Confirm File Delete", "act_off_":"Confirm Channel OFF", "act_on_":"Confirm Channel ON",
    "edopt_":"Edit File Option", "ef_t_":"Edit File Title", "ef_c_":"Edit File Code", "ef_p_":"Edit File Post",
    "fje_":"Edit Force Join Slot", "f_del_page_":"File Delete Pagination", "f_edit_page_":"File Edit Pagination", "f_list_page_":"File List Pagination"
}
BUTTON_COLOR_ROLES = {"fj_channel":"Force Join Channel URL Buttons", "notice_join_now":"JOIN NOW URL Button", "update_button":"Update Channel URL Button"}

def _button_setting_key(key):
    return "button_color_" + str(key).replace(".", "_")

def get_button_style_for_key(key, default=None):
    value = get_setting(_button_setting_key(key), default or get_setting("button_color_default", "primary"))
    value = str(value or "default").lower()
    return value if value in BUTTON_STYLE_CHOICES else "default"

def _button_style_key(btn):
    cb = btn.get("callback_data")
    if cb:
        cb = str(cb)
        if _button_setting_key(cb) in _SETTINGS_CACHE:
            return cb
        for prefix in BUTTON_COLOR_PREFIX_LABELS:
            if cb.startswith(prefix): return prefix
        return cb
    return btn.get("_style_role")

def apply_button_styles(markup):
    if not isinstance(markup, dict) or "inline_keyboard" not in markup: return markup
    out = dict(markup); rows=[]
    for row in markup.get("inline_keyboard") or []:
        new=[]
        for original in row or []:
            btn=dict(original); key=_button_style_key(btn)
            if key:
                style=get_button_style_for_key(key)
                if style == "default": btn.pop("style", None)
                else: btn["style"] = style
            btn.pop("_style_role", None); new.append(btn)
        rows.append(new)
    out["inline_keyboard"] = rows
    return out

def _style_role_button(text, role, **kwargs):
    b={"text":text, **kwargs}; b["_style_role"]=role; return b

def _button_color_catalog():
    catalog=list(BUTTON_COLOR_LABELS.items()) + list(BUTTON_COLOR_PREFIX_LABELS.items()) + list(BUTTON_COLOR_ROLES.items())
    return list(dict(catalog).items())

def _button_color_status(key): return get_button_style_for_key(key)

def show_button_colors(cid, mid, page=0):
    catalog=_button_color_catalog(); page_size=12; page=max(0,int(page)); start=page*page_size
    kb=[]
    for key,label in catalog[start:start+page_size]:
        style=_button_color_status(key); icon={"primary":"🔵","success":"🟢","danger":"🔴","default":"⚪"}.get(style,"⚪")
        kb.append([{"text":f"{icon} {label}: {style.upper()}","callback_data":f"bc_pick_{key}"}])
    nav=[]
    if page>0: nav.append({"text":"◀️ Previous","callback_data":f"bc_list_{page-1}"})
    if start+page_size<len(catalog): nav.append({"text":"Next ▶️","callback_data":f"bc_list_{page+1}"})
    if nav: kb.append(nav)
    kb.append([{"text":"⬅️ "+mono("Back to Buttons"),"callback_data":"set_grp_buttons"}])
    tg_call("editMessageText",{"chat_id":cid,"message_id":mid,"text":f"🎨 **BUTTON COLORS**\n\nChoose any button and set its color.\n\n🔵 Primary  🟢 Success  🔴 Danger  ⚪ Default\n\nPage {page+1}/{max(1,(len(catalog)+page_size-1)//page_size)}","parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}})

def show_button_color_picker(cid, mid, key):
    label=dict(_button_color_catalog()).get(key,key); current=_button_color_status(key)
    kb=[[{"text":"🔵 Primary","callback_data":f"bc_set_{key}_primary"},{"text":"🟢 Success","callback_data":f"bc_set_{key}_success"}],
        [{"text":"🔴 Danger","callback_data":f"bc_set_{key}_danger"},{"text":"⚪ Default","callback_data":f"bc_set_{key}_default"}],
        [{"text":"⬅️ "+mono("Back"),"callback_data":"bc_back"}]]
    tg_call("editMessageText",{"chat_id":cid,"message_id":mid,"text":f"🎨 **{label}**\n\nCurrent color: `{current.upper()}`\n\nSelect the new button color:","parse_mode":"Markdown","reply_markup":{"inline_keyboard":kb}})

def tg_call(method, data):
    try:
        styled_data=dict(data or {})
        if isinstance(styled_data.get("reply_markup"),dict): styled_data["reply_markup"]=apply_button_styles(styled_data["reply_markup"])
        r=_TG_SESSION.post(f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",json=styled_data,timeout=(2.5,8)).json()
        if not r.get("ok"):
            retry_data=dict(styled_data)
            if isinstance(retry_data.get("reply_markup"),dict):
                rm=dict(retry_data["reply_markup"]); rm["inline_keyboard"]=[[{k:v for k,v in dict(btn).items() if k!="style"} for btn in row] for row in rm.get("inline_keyboard") or []]; retry_data["reply_markup"]=rm
            if "parse_mode" in retry_data: retry_data.pop("parse_mode",None)
            return _TG_SESSION.post(f"https://api.telegram.org/bot{BOT_TOKEN}/{method}",json=retry_data,timeout=(2.5,8)).json()
        return r
...
def _fj_keyboard(unjoined, code):
    ...
    for c in unjoined:
        ...
        row.append(_style_role_button(btn_txt, "fj_channel", url=c.get("join_url")))
        if len(row) == layout:
            kb.append(row); row = []
...
def deliver_file(chat_id, code):
    ...
    if del_seconds > 0 and (cleanup_file or cleanup_notice):
        ...
        kb = [[_style_role_button(btn_txt, "notice_join_now", url=btn_url)]] if (notice_btn_enabled and btn_txt and btn_url and btn_txt.lower() != "none") else []
        w = {}
...
def show_settings_menu(chat_id, mid=None):
    ...
    kb=[
        ...
        [_settings_button("🎨 Button Colors","set_grp_button_colors","bold")],
...
def show_settings_group(cid, mid, group):
    ...
    if group == "fj":
        ...
        kb=[[{"text":"🎨 "+mono("Button Colors"),"callback_data":"set_button_colors"}],
            [{"text":"🔘 "+mono("Force Join Layout"),"callback_data":"s_layout"}],
...
def process_callback_query(cq):
    ...
    if uid in ADMIN_IDS:
        ...
        elif cdata == "set_grp_button_colors":
            show_button_colors(cid, mid, 0)
        ...
        # --- Per-button color settings ---
        elif cdata == "set_button_colors":
            show_button_colors(cid, mid, 0)
        elif cdata.startswith("bc_list_"):
            try: show_button_colors(cid, mid, int(cdata.split("_")[-1]))
            except Exception: show_button_colors(cid, mid, 0)
        elif cdata.startswith("bc_pick_"):
            show_button_color_picker(cid, mid, cdata[len("bc_pick_"):])
        elif cdata.startswith("bc_set_"):
            raw=cdata[len("bc_set_"):]
            if "_" in raw:
                key,style=raw.rsplit("_",1)
                if style in BUTTON_STYLE_CHOICES: set_setting(_button_setting_key(key),style)
            show_button_colors(cid, mid, 0)
        elif cdata == "bc_back":
            show_button_colors(cid, mid, 0)
